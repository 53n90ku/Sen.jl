...
function run_batch_search_benchmark(db::VectorDB,queries::AbstractMatrix{<:Real};repetitions::Int=10,workers::Int=Threads.nthreads(:default),parallel_threshold::Int=4,kwargs...)
    ...
    serial=measure_latency(
        ()->search(db,queries;parallel=false,workers=1,parallel_threshold=parallel_threshold,kwargs...);
        repetitions=repetitions,
    )
    parallel=measure_latency(
        ()->search(db,queries;parallel=true,workers=workers,parallel_threshold=parallel_threshold,kwargs...);
        repetitions=repetitions,
    )
    ...
    return(
        ...
        workers=Sen.resolve_database_batch_workers(query_count;parallel=true,workers=workers,parallel_threshold=parallel_threshold,),
