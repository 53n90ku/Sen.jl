...
function manifest_preprocessing_hash(
    name::AbstractString,
...
function create_dataset_manifest(
    path::AbstractString;
    name::AbstractString,
    ...
    preprocessing_hash=manifest_preprocessing_hash(
        name,
        revision,
        vector_count,
        dimension,
...
function validate_dataset_manifest(
    ...
    for (filename, expected_size) in manifest.file_sizes
        ...
        filesize(file_path)==expected_size||throw(
            ArgumentError("dataset file size changed: $(filename)"),
        )
        verify_hashes||continue
        file_sha256(file_path)==manifest.files[filename]||throw(
            ArgumentError("dataset file hash changed: $(filename)"),
        )
    end

    expected=manifest_preprocessing_hash(
        manifest.name,
        manifest.revision,
    ...
    expected==manifest.preprocessing_hash||throw(
        ArgumentError("dataset preprocessing hash is invalid"),
    )
    return manifest
end
