...
function generate_clustered_dataset(
    n::Int,
    dim::Int;
    clusters::Int = 16,
    cluster_noise::Float64 = 0.10,
    cluster_skew::Float64 = 0.0,
    seed::Int = 42,
)
    n>0||throw(ArgumentError("n must be positive"))
    dim>0||throw(ArgumentError("dim must be positive"))
    ...
    weights=[1/index^cluster_skew for index = 1:clusters]
    assignments=Vector{Int}(undef, n)
    vectors=Matrix{Float32}(undef, dim, n)

    for vector_index = 1:n
        cluster=vector_index<=clusters ? vector_index : sample_cluster(rng, weights)
        assignments[vector_index]=cluster

        for dimension = 1:dim
            vectors[dimension, vector_index]=centers[dimension, cluster]+Float32(
                cluster_noise*randn(rng),
            )
        end
    end

    normalize_columns!(vectors)
    metadata=[(cluster = assignments[index],) for index = 1:n]
...
function generate_clustered_queries(
    count::Int,
    centers::AbstractMatrix;
    query_noise::Float64 = 0.10,
    cluster_skew::Float64 = 0.0,
    seed::Int = 43,
)
    ...
    rng=MersenneTwister(seed)
    weights=[1/index^cluster_skew for index = 1:cluster_count]
    assignments=Vector{Int}(undef, count)
    queries=Matrix{Float32}(undef, dim, count)

    for query_index = 1:count
        cluster=sample_cluster(rng, weights)
        assignments[query_index]=cluster

        for dimension = 1:dim
            queries[dimension, query_index]=centers[dimension, cluster]+Float32(
                query_noise*randn(rng),
            )
        end
    end
...
function load_fvecs(
    ...
    return open(path, "r") do io
        ...
        filesize(path)%record_size==0||throw(
            ArgumentError("fvecs file size does not match its dimension"),
        )
