...
function groundtruth_cache_signature(dataset_hash::AbstractString,query_indices::AbstractVector{<:Integer},k::Int,metric::Symbol,filter_name::AbstractString)
...
function save_ivecs(path::AbstractString,rows::AbstractVector{<:AbstractVector{<:Integer}};zero_based::Bool=true,)
    ...
    all(row->length(row)==dimension,rows)||throw(DimensionMismatch("groundtruth rows must have one dimension"))
    mkpath(dirname(path))
...
function save_groundtruth_cache(path::AbstractString,rows::AbstractVector{<:AbstractVector{<:Integer}};dataset_hash::AbstractString,query_indices::AbstractVector{<:Integer},k::Int,metric::Symbol=:cosine,filter_name::AbstractString="none",)
    length(rows)==length(query_indices)||throw(DimensionMismatch("groundtruth rows do not match query indices"))
    data_path="$(path).ivecs"
    manifest_path="$(path).toml"
    signature=groundtruth_cache_signature(dataset_hash,query_indices,k,metric,filter_name)
...
function load_groundtruth_cache(path::AbstractString;dataset_hash::AbstractString,query_indices::AbstractVector{<:Integer},k::Int,metric::Symbol=:cosine,filter_name::AbstractString="none",)
    data_path="$(path).ivecs"
    manifest_path="$(path).toml"
    isfile(data_path)&&isfile(manifest_path)||throw(ArgumentError("groundtruth cache does not exist"))
    data=TOML.parsefile(manifest_path)
    expected=groundtruth_cache_signature(dataset_hash,query_indices,k,metric,filter_name)
    String(data["signature"])==expected||throw(ArgumentError("groundtruth cache is stale"))
    file_sha256(data_path)==String(data["data_sha256"])||throw(ArgumentError("groundtruth cache data changed"))
