...
function ivf_vector_lists(index::IVFIndex)
    ...
    for(list_index,list) in enumerate(index.lists)
        for vector_index in list
            ...
            iszero(assignments[vector_index])||throw(ArgumentError("vector occurs in more than one IVF list"))
            assignments[vector_index]=list_index
        end
    end
    all(value->!iszero(value),assignments)||throw(ArgumentError("IVF index does not assign every vector"))
    return assignments
end

function ivf_index_diagnostics(index::IVFIndex,vectors::AbstractMatrix)
    size(vectors,2)==sum(length,index.lists)||throw(DimensionMismatch("vectors do not match IVF index"))
    size(vectors,1)==size(index.centroids,1)||throw(DimensionMismatch("vector dimensions do not match IVF index"))
    ...
    for vector_index in eachindex(assignments)
        ...
        losses[vector_index]=index.metric===:cosine ? 1-clamp(similarity,-1,1) : -similarity
    end
    ...
    return(
        ...
        maximum_to_average_list_ratio=iszero(average_size) ? 0.0 : maximum(list_sizes)/average_size,
...
function ivf_routing_diagnostics(index::IVFIndex,queries::AbstractMatrix,truth_ids::AbstractVector;metadata=nothing,filters=nothing,)
    size(queries,1)==size(index.centroids,1)||throw(DimensionMismatch("queries do not match IVF index"))
    size(queries,2)==length(truth_ids)||throw(DimensionMismatch("truth count does not match queries"))
    filters===nothing||length(filters)==size(queries,2)||throw(DimensionMismatch("filter count does not match queries"))
    metadata===nothing||length(metadata)==sum(length,index.lists)||throw(DimensionMismatch("metadata does not match IVF index"))
    assignments=ivf_vector_lists(index)
    selectivities=metadata===nothing||filters===nothing ? fill(NaN,size(queries,2)) : dataset_selectivities(metadata,filters)
    rows=NamedTuple[]
    ...
    for query_index in axes(queries,2)
        ...
        push!(rows,(
            ...
            truth_list_count=length(unique(assignments[vector_index] for vector_index in truth)),
...
function ivf_probe_curve(index::IVFIndex,queries::AbstractMatrix,truth_ids::AbstractVector,nprobes::AbstractVector{<:Integer};metadata=nothing,filters=nothing,)
    routing=ivf_routing_diagnostics(index,queries,truth_ids;metadata=metadata,filters=filters,)
    assignments=ivf_vector_lists(index)
    rows=NamedTuple[]
    ...
    for query_index in axes(queries,2)
        ...
        matching_sizes=metadata===nothing||filters===nothing ? copy(list_sizes) : Int[count(index->matches_filter(metadata[index],filters[query_index]),index.lists[list_index]) for list_index in ranking]
        ...
        for nprobe in nprobes
            ...
            push!(rows,merge(routing[query_index],(
                nprobe=nprobe,
                ...
                database_fraction=cumulative_sizes[nprobe]/sum(length,index.lists),
            )))
        end
    end
