...
function ivf_sweep_configuration_hash(dataset_hash::AbstractString,query_indices::AbstractVector{<:Integer};nlists::Int,training_count::Int,iterations::Int,restarts::Int,seed::Int,metric::Symbol=:cosine,)
    value=join((dataset_hash,join(query_indices,','),nlists,training_count,iterations,restarts,seed,metric),'|')
    return bytes2hex(SHA.sha256(value))
end

function sweep_configuration_name(configuration_hash::AbstractString;nlists::Int,training_count::Int,iterations::Int,restarts::Int,seed::Int,)
    return "lists-$(nlists)-train-$(training_count)-iter-$(iterations)-restart-$(restarts)-seed-$(seed)-$(configuration_hash[1:12])"
end

function sweep_checkpoint_data(dataset::ArxivFANNSDataset,configuration_hash::String,index_diagnostics,aggregate_rows;nlists::Int,training_count::Int,iterations::Int,restarts::Int,seed::Int,build_seconds::Float64,)
    eligible=[row for row in aggregate_rows if row.bucket===:all&&row.recall>=0.97]
    selected=isempty(eligible) ? first(sort([row for row in aggregate_rows if row.bucket===:all];by=row->(-row.recall,row.average_candidates,row.nprobe),)) : first(sort(eligible;by=row->(row.average_candidates,row.nprobe),))
    return Dict(
        "complete"=>true,
...
function run_ivf_quality_configuration(dataset::ArxivFANNSDataset,output_path::AbstractString;nlists::Int,training_count::Int,iterations::Int=8,restarts::Int=2,seed::Int=dataset.manifest.sampling_seed,force::Bool=false,)
    nlists<=training_count<=size(dataset.vectors,2)||throw(ArgumentError("training count must be between nlists and vector count"))
    configuration_hash=ivf_sweep_configuration_hash(dataset.manifest.preprocessing_hash,dataset.train_indices;nlists=nlists,training_count=training_count,iterations=iterations,restarts=restarts,seed=seed,)
    name=sweep_configuration_name(configuration_hash;nlists=nlists,training_count=training_count,iterations=iterations,restarts=restarts,seed=seed,)
    ...
    if isfile(checkpoint_path)&&isfile(joinpath(index_path,"index.bin"))&&!force
        ...
        get(checkpoint,"complete",false)||throw(ArgumentError("incomplete IVF sweep checkpoint exists at $(path)"))
        String(checkpoint["configuration_hash"])==configuration_hash||throw(ArgumentError("IVF sweep checkpoint configuration changed"))
        return(status=:cached,path=path,checkpoint=checkpoint,)
    end

    mkpath(path)
    index=nothing
    build_seconds=@elapsed index=build_ivf(dataset.vectors;nlists=nlists,iterations=iterations,restarts=restarts,training_count=training_count,seed=seed,metric=:cosine,)
    index_diagnostics=ivf_index_diagnostics(index,dataset.vectors)
    nprobes=probe_schedule(nlists)
    routing_rows=ivf_routing_diagnostics(index,dataset.train_queries,dataset.train_truth;metadata=dataset.metadata,filters=dataset.train_filters,)
    curve_rows=ivf_probe_curve(index,dataset.train_queries,dataset.train_truth,nprobes;metadata=dataset.metadata,filters=dataset.train_filters,)
    ...
    checkpoint=sweep_checkpoint_data(dataset,configuration_hash,index_diagnostics,aggregate_rows;nlists=nlists,training_count=training_count,iterations=iterations,restarts=restarts,seed=seed,build_seconds=build_seconds,)
...
function run_ivf_quality_sweep(dataset::ArxivFANNSDataset,output_path::AbstractString;nlists_values::AbstractVector{<:Integer}=[64,128,256,512],training_count::Int=20_000,iterations::Int=8,restarts::Int=2,seed::Int=dataset.manifest.sampling_seed,force::Bool=false,)
    mkpath(output_path)
    results=NamedTuple[]
    for nlists in nlists_values
        println("IVF sweep nlists=$(nlists) training=$(training_count) iterations=$(iterations) restarts=$(restarts)")
        result=run_ivf_quality_configuration(dataset,output_path;nlists=Int(nlists),training_count=training_count,iterations=iterations,restarts=restarts,seed=seed,force=force,)
...
function bootstrap_recall_interval(rows::AbstractVector;samples::Int=2_000,seed::Int=42,confidence::Float64=0.95,)
    ...
    for sample in 1:samples
        estimates[sample]=sum(recalls[rand(rng,1:length(recalls))] for _ in eachindex(recalls))/length(recalls)
    end
    alpha=(1-confidence)/2
    return(lower=percentile(estimates,alpha),estimate=sum(recalls)/length(recalls),upper=percentile(estimates,1-alpha),confidence=confidence,samples=samples,)
end

function evaluate_hybrid_validation(dataset::ArxivFANNSDataset,index::IVFIndex;nprobe::Int,k::Int=10,repetitions::Int=3,rare_threshold::Float64=0.01,target_recall::Float64=0.95,split::Symbol=:validation,)
    0<=rare_threshold<=1||throw(ArgumentError("rare threshold must be between zero and one"))
    1<=nprobe<=length(index.lists)||throw(ArgumentError("nprobe must be between one and list count"))
    spec=real_experiment_spec(dataset;name="ivf-hybrid-validation",nlists=length(index.lists),nprobes=[nprobe],k=k,repetitions=repetitions,target_recall=target_recall,training_count=size(dataset.vectors,2),iterations=1,restarts=1,)
    context=build_benchmark_context(index,dataset.metadata)
    exact=evaluate_benchmark_method(spec,context,dataset.vectors,dataset.metadata,dataset.heldout_queries,dataset.heldout_filters,dataset.heldout_truth,:exact,0;split=split,)
    approximate=evaluate_benchmark_method(spec,context,dataset.vectors,dataset.metadata,dataset.heldout_queries,dataset.heldout_filters,dataset.heldout_truth,:ivf_prefilter,nprobe;split=split,)
    ...
    approximate_rows=Dict((row.query_index,row.repetition)=>row for row in approximate.rows)
    ...
    for bucket in (:rare,:medium,:broad)
        ...
        isempty(bucket_rows)||setindex!(bucket_summaries,evaluation_from_rows(bucket_rows),bucket)
    end
    ...
    return(
        ...
        recall_interval=bootstrap_recall_interval(hybrid_rows;seed=dataset.manifest.sampling_seed,),
        bucket_intervals=Dict(bucket=>bootstrap_recall_interval([row for row in hybrid_rows if row.bucket==String(bucket)];seed=dataset.manifest.sampling_seed+index,) for(index,bucket) in enumerate(keys(bucket_summaries))),
...
function save_hybrid_validation(path::AbstractString,result)
    ...
    for(method,evaluation) in ((:exact,result.exact),(:ivf_prefilter,result.approximate),(:hybrid,result.hybrid))
    ...
    for(bucket,summary) in sort!(collect(result.buckets);by=value->string(first(value)),)
        push!(summary_rows,(
