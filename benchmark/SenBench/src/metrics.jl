function recall_at_k(
    predicted_ids::AbstractVector{<:Integer},
    truth_ids::AbstractVector{<:Integer},
    k::Int,
)::Float64
...
function latency_summary(times_ns::AbstractVector{<:Integer})
    ...
    any(time->time<0, times_ns)&&throw(
        ArgumentError("latency measurements cannot be negative"),
    )
...
function count_exact_candidates(
    index::BitsetIndex,
...
function count_ivf_prefilter_work(
    index::IVFIndex,
    ...
    return (
        visited = length(visited_indices),
        scored = length(candidate_indices),
        probed_lists = length(selected_lists),
    )
end
...
function count_ivf_prefilter_work(
    ...
    length(metadata)==sum(length, index.ivf.lists)||throw(
    ...
    return (
        visited = length(candidate_indices),
        scored = length(candidate_indices),
        probed_lists = length(selected_lists),
    )
end

function count_filter_aware_work(
    index::FilterAwareIVFIndex,
    metadata::AbstractVector,
    query::AbstractVector,
    filter::Union{NamedTuple,FilterExpr};
