...
function quality_workload_spec(table::AbstractDict)
    ...
    filter_workload in (:none, :selected)||throw(
        ArgumentError("$context has an unsupported filter workload"),
    )
    ...
    heldout_query_count>0||throw(
        ArgumentError("$context heldout query count must be positive"),
    )
    1<=nlists<=vector_count||throw(ArgumentError("$context list count is invalid"))
    !isempty(nprobes)&&all(probe->1<=probe<=nlists, nprobes)||throw(
        ArgumentError("$context nprobes are invalid"),
    )
    maximum(nprobes)==nlists||throw(
        ArgumentError("$context must include a full-probe recall point"),
    )
    ...
    probe_safety_factor>=1.0||throw(
        ArgumentError("$context probe safety factor must be at least one"),
    )
    ...
    !isempty(methods)&&Set(methods)⊆QUALITY_METHODS||throw(
        ArgumentError("$context methods are invalid"),
    )
    length(methods)==length(Set(methods))||throw(
        ArgumentError("$context methods must be unique"),
    )
    filter_workload===:none&&methods!=[:ivf]&&throw(
        ArgumentError("$context unfiltered workloads must measure IVF"),
    )
    filter_workload===:selected&&(:ivf in methods)&&throw(
        ArgumentError("$context filtered workloads cannot use unfiltered IVF"),
    )
    metric===:dot&&(:filter_aware_bound in methods)&&throw(
        ArgumentError("$context dot workloads cannot use the cosine bound strategy"),
    )
    all(
        value->isempty(value)||occursin(r"^[0-9a-f]{64}$", value),
        (dataset_sha256, workload_sha256),
    )||throw(ArgumentError("$context fingerprints are invalid"))

    return QualityWorkloadSpec(
        id,
        metric,
        filter_workload,
        vector_count,
...
function load_quality_contract(path::AbstractString)
    ...
    get(data, "format_version", nothing)==1||throw(
        ArgumentError("unsupported quality contract format"),
    )
    contract_version=VersionNumber(
        String(quality_required(data, "contract_version", "quality contract")),
    )
    index_version=Int(quality_required(data, "index_version", "quality contract"))
    generator_version=String(
        quality_required(data, "generator_version", "quality contract"),
    )
    index_version==Sen.IVF_INDEX_VERSION||throw(
        ArgumentError("quality contract index version does not match Sen"),
    )
    !isempty(generator_version)||throw(
        ArgumentError("quality contract generator version cannot be empty"),
    )
    raw_workloads=quality_required(data, "workloads", "quality contract")
    raw_workloads isa AbstractVector&&!isempty(raw_workloads)||throw(
        ArgumentError("quality contract workloads cannot be empty"),
    )
    workloads=QualityWorkloadSpec[quality_workload_spec(table) for table in raw_workloads]
    length(workloads)==length(Set(spec.id for spec in workloads))||throw(
        ArgumentError("quality workload IDs must be unique"),
    )
...
function generate_quality_dataset(spec::QualityWorkloadSpec)
    ...
    metadata=[
        (
            selected = dataset.assignments[index]<=selected_clusters,
            cluster = dataset.assignments[index],
        ) for index = 1:spec.vector_count
    ]
...
function quality_dataset_fingerprint(spec::QualityWorkloadSpec, data)
    ...
    write(
        io,
        Int64(spec.vector_count),
        Int64(spec.dimension),
        Int64(spec.train_query_count),
        Int64(spec.heldout_query_count),
        Int64(spec.seed),
    )
...
function quality_workload_fingerprint(
    contract::QualityContract,
    spec::QualityWorkloadSpec,
    dataset_sha256::AbstractString,
)
    value=join(
        (
...
function quality_search(
    ...
    elseif method===:ivf_postfilter
        return search(
            ...
            nprobe = nprobe,
            filter = filter,
            adaptive_postfilter = false,
            postfilter_oversample = 64,
        )
...
function quality_truth(
    ...
    return [
        [
            result.id for result in search(
                db,
                @view(queries[:, index]);
                k = spec.k,
                strategy = :exact,
...
function quality_average_recall(
    db::VectorDB,
    queries::AbstractMatrix,
    truth,
    spec::QualityWorkloadSpec,
    ...
    for index in axes(queries, 2)
        predicted=Int[
            ...
            quality_search(db, @view(queries[:, index]), spec, method, nprobe, filter)
        ]
...
function select_quality_nprobe(
    db::VectorDB,
    data,
    spec::QualityWorkloadSpec,
    method::Symbol,
    ...
    return (
        ...
        train_gate = base.recall>=spec.target_recall,
    )
end

function quality_latency(
    db::VectorDB,
    queries::AbstractMatrix,
    spec::QualityWorkloadSpec,
    method::Symbol,
...
function run_quality_workload(
    ...
    if verify_fingerprints
        dataset_sha256==spec.dataset_sha256||throw(
            ArgumentError("quality workload $(spec.id) dataset fingerprint changed"),
        )
        workload_sha256==spec.workload_sha256||throw(
            ArgumentError("quality workload $(spec.id) contract fingerprint changed"),
        )
    end
    ...
    db=create_db(
        ...
        metric = spec.metric,
        durable = false,
        maintenance_config = MaintenanceConfig(enabled = false),
    )
    ...
    try
        ...
        for method in spec.methods
            ...
            heldout_recall=quality_average_recall(
                db,
                data.heldout_queries,
                heldout_truth,
                spec,
                method,
                selection.selected_nprobe,
                data.filter,
            )
            latency=quality_latency(
                db,
                data.heldout_queries,
                spec,
                method,
        ...
        return (
            ...
            max_p95_ms = spec.max_p95_ms,
            methods = results,
            passed = all(result->result.recall_passed&&result.latency_passed, results),
        )
    finally
        close(db)
    end
...
function validate_quality_contract(
    path::AbstractString;
    ...
    methods==QUALITY_METHODS||throw(
        ArgumentError("quality contract does not cover every approximate strategy"),
    )
    Set(spec.metric for spec in contract.workloads)==Set([:cosine, :dot])||throw(
        ArgumentError("quality contract must cover cosine and dot metrics"),
    )
    all(
        metric->any(
        ...
        ArgumentError("quality contract must include unfiltered cosine and dot workloads"),
    )
    run_benchmarks||return (contract = contract, results = NamedTuple[], passed = true)
    results=[
        run_quality_workload(contract, spec; verify_fingerprints = verify_fingerprints) for
        spec in contract.workloads
    ]
    return (
        contract = contract,
        results = results,
        passed = all(result->result.passed, results),
    )
end
