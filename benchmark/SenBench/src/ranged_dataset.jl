...
function ranged_subset_url(filename::AbstractString;revision::AbstractString=ARXIV_LARGE_REVISION,)
...
function load_ranged_state(path::AbstractString,spec::RangedSubsetSpec)
    ...
    String(data["revision"])==spec.revision||throw(ArgumentError("ranged source revision changed"))
    String(data["vector_etag"])==spec.vector_etag||throw(ArgumentError("ranged vector etag changed"))
    Int(data["dimension"])==spec.dimension||throw(DimensionMismatch("ranged vector dimension changed"))
    Int(data["chunk_records"])==spec.chunk_records||throw(ArgumentError("ranged chunk size changed"))
    return data
end

function download_exact_range(url::AbstractString,path::AbstractString,first_byte::Int,last_byte::Int)
    ...
    command=Cmd([curl,"--location","--fail","--silent","--show-error","--retry","4","--retry-all-errors","--retry-delay","2","--connect-timeout","30","--range","$(first_byte)-$(last_byte)","--max-filesize",string(expected),"--output",String(path),String(url)])
    for attempt in 1:6
        isfile(path)&&rm(path)
...
function append_normalized_fvecs!(output_path::AbstractString,chunk_path::AbstractString,dimension::Int,record_count::Int)
    record_size=sizeof(Int32)+dimension*sizeof(Float32)
    filesize(chunk_path)==record_count*record_size||throw(DimensionMismatch("fvecs chunk size is invalid"))
    ...
    open(output_path,"a") do output
        open(chunk_path,"r") do input
            for _ in 1:record_count
                ...
                stored_dimension==dimension||throw(DimensionMismatch("fvecs chunk dimension changed"))
                read!(input,vector)
...
function extend_ranged_vectors!(path::AbstractString,spec::RangedSubsetSpec)
    ...
    completed<=spec.vector_count||throw(ArgumentError("ranged state exceeds requested vector count"))
    expected_output=completed*spec.dimension*sizeof(Float32)

    if isfile(output_path)
        filesize(output_path)>=expected_output||throw(ArgumentError("ranged vector output is truncated"))
    ...
    filesize(output_path)==spec.vector_count*spec.dimension*sizeof(Float32)||throw(DimensionMismatch("ranged vector output size is invalid"))
    return(output=output_path,state=state_path,sha256=file_sha256(output_path),)
end

function copy_jsonl_prefix(source_path::AbstractString,output_path::AbstractString,count::Int)
    ...
    written==count||throw(ArgumentError("metadata source contains fewer rows than requested"))
...
function load_f32_matrix(path::AbstractString,dimension::Int,count::Int)
    ...
    isfile(path)&&filesize(path)==expected||throw(DimensionMismatch("f32 matrix size is invalid"))
...
function subset_dataset_manifest(path::AbstractString,spec::RangedSubsetSpec)
    filenames=["database_vectors.f32","database_attributes.jsonl","query_vectors.fvecs","em_query_attributes.jsonl"]
...
function prepare_arxiv_subset(path::AbstractString;vector_count::Int=300_000,chunk_records::Int=4_096,seed::Int=42,revision::AbstractString=ARXIV_LARGE_REVISION,keep_metadata_source::Bool=true,)
    1<=vector_count<=ARXIV_FANNS_COUNTS[:large]||throw(ArgumentError("subset vector count is invalid"))
    ...
    mkpath(path)
    existing=isfile(joinpath(path,"database_vectors.f32")) ? filesize(joinpath(path,"database_vectors.f32")) : 0
    needed=max(0,vector_count*spec.dimension*sizeof(Float32)-existing)+ARXIV_LARGE_METADATA_BYTES+200_000_000+2*1024^3
    disk_available_bytes(path)>=needed||throw(ArgumentError("insufficient disk space for safe subset preparation"))
    vectors=extend_ranged_vectors!(path,spec)
    metadata_source=joinpath(path,"database_attributes.source.jsonl")
    download_file_resumable(ranged_subset_url("database_attributes.jsonl";revision=spec.revision,),metadata_source)
    copy_jsonl_prefix(metadata_source,joinpath(path,"database_attributes.jsonl"),vector_count)
    keep_metadata_source||rm(metadata_source;force=true,)
    download_file_resumable(ranged_subset_url("query_vectors.fvecs";revision=spec.revision,),joinpath(path,"query_vectors.fvecs"))
    download_file_resumable(ranged_subset_url("em_query_attributes.jsonl";revision=spec.revision,),joinpath(path,"em_query_attributes.jsonl"))
    manifest=subset_dataset_manifest(path,spec)
    return(spec=spec,manifest=manifest,vectors=vectors,)
end

function load_arxiv_subset(path::AbstractString,train_indices::AbstractVector{<:Integer},heldout_indices::AbstractVector{<:Integer};k::Int=10,groundtruth=nothing,verify_hashes::Bool=false,groundtruth_block_size::Int=2_048,)
    ...
    vectors=load_f32_matrix(joinpath(path,"database_vectors.f32"),manifest.dimension,manifest.vector_count)
    ...
    truths=groundtruth===nothing ? load_or_compute_blockwise_groundtruth(path,manifest,vectors,metadata,queries,filters,all_indices;k=k,metric=:cosine,block_size=groundtruth_block_size,) : groundtruth
    length(truths)==length(all_indices)||throw(DimensionMismatch("groundtruth count does not match queries"))
