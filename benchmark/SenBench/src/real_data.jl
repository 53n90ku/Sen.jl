...
const ARXIV_FANNS_COUNTS=Dict(:small=>1_000, :medium=>100_000, :large=>2_735_264)
...
Base.:(==)(left::RealQuerySplit, right::RealQuerySplit) =
    left.train_indices==right.train_indices&&left.heldout_indices==right.heldout_indices
...
Base.:(==)(left::RealQueryPartitions, right::RealQueryPartitions) =
    left.development_indices==right.development_indices&&left.validation_indices==right.validation_indices&&left.confirmation_indices==right.confirmation_indices&&left.confirmation_count==right.confirmation_count&&left.confirmation_hash==right.confirmation_hash&&left.seed==right.seed
...
function arxiv_fanns_repository(scale::Symbol)
    haskey(ARXIV_FANNS_COUNTS, scale)||throw(
        ArgumentError("scale must be small, medium or large"),
    )
...
function download_arxiv_fanns(path::AbstractString; scale::Symbol = :medium)
    ...
    for filename in ARXIV_FANNS_FILES
        ...
        downloaded[filename]=download_file_resumable(
            arxiv_fanns_url(scale, filename),
...
function vector_file_info(path::AbstractString, stored_type::Type)
    ...
    return open(path, "r") do io
        ...
        filesize(path)%record_size==0||throw(
            ArgumentError("vector file size does not match its dimension"),
        )
        return (
            dimension = dimension,
            count = filesize(path)÷record_size,
...
function ivecs_info(path::AbstractString)
    ...
    open(path, "r") do io
        while !eof(io)
            ...
            dimension*sizeof(Int32)<=remaining||throw(
                ArgumentError("ivecs record is truncated"),
            )
    ...
    return (
        ...
        count = count,
        variable = minimum_dimension!=maximum_dimension,
    )
end

function load_fvecs_indices(
    path::AbstractString,
    ...
    open(path, "r") do io
        for (output_index, source_index) in enumerate(resolved)
            ...
            stored_dimension==info.dimension||throw(
                DimensionMismatch("fvecs vectors must have one dimension"),
            )
...
function load_arxiv_metadata(path::AbstractString; limit::Union{Nothing,Int} = nothing)
    ...
    limit===nothing||length(metadata)==limit||throw(
        ArgumentError("metadata contains fewer rows than requested"),
    )
    return metadata
end
...
function stratified_query_split(
    ...
    train_count+heldout_count<=length(labels)||throw(
        ArgumentError("not enough queries for split"),
    )
...
function stratified_query_partitions(
    labels::AbstractVector{<:Integer};
    development_count::Int = 256,
    validation_count::Int = 128,
    confirmation_count::Int = 256,
    seed::Int = 42,
    strata::AbstractVector{<:Integer} = labels,
)
    ...
    length(strata)==length(labels)||throw(
        DimensionMismatch("query strata do not match labels"),
    )
    first_split=stratified_query_split(
        strata;
        train_count = development_count,
        heldout_count = validation_count+confirmation_count,
        seed = seed,
    )
    heldout_strata=strata[first_split.heldout_indices]
    second_split=stratified_query_split(
        heldout_strata;
        train_count = validation_count,
        heldout_count = confirmation_count,
        seed = seed+1,
    )
...
function proportional_allocation(total::Int, capacities::AbstractVector{<:Integer})
    ...
    order=sortperm(
        eachindex(capacities);
        by = index->(raw[index]-allocation[index], capacities[index]-allocation[index]),
        rev = true,
    )
...
function balanced_query_partitions(
    labels::AbstractVector{<:Integer};
    development_count::Int = 256,
    validation_count::Int = 128,
    confirmation_count::Int = 256,
    ...
    for stratum in keys_order
        ...
        allocation=available==sum(target_per_stratum) ? copy(target_per_stratum) :
                   proportional_allocation(available, counts)
        position=1
...
function validate_query_partitions(partitions::RealQueryPartitions, query_count::Int)
    ...
    all(index->1<=index<=query_count, public_indices)||throw(
        BoundsError(1:query_count, public_indices),
    )
    isempty(
        intersect(Set(partitions.development_indices), Set(partitions.validation_indices)),
    )||throw(ArgumentError("development and validation queries overlap"))

    if !isempty(partitions.confirmation_indices)
        length(partitions.confirmation_indices)==partitions.confirmation_count||throw(
            DimensionMismatch("confirmation query count changed"),
        )
        all(index->1<=index<=query_count, partitions.confirmation_indices)||throw(
            BoundsError(1:query_count, partitions.confirmation_indices),
        )
        isempty(intersect(Set(public_indices), Set(partitions.confirmation_indices)))||throw(
            ArgumentError("confirmation queries overlap public partitions"),
        )
        actual_hash=bytes2hex(SHA.sha256(join(partitions.confirmation_indices, ',')))
        actual_hash==partitions.confirmation_hash||throw(
            ArgumentError("confirmation query hash changed"),
        )
    end
...
function save_query_partitions(
    path::AbstractString,
    partitions::RealQueryPartitions;
    dataset_hash::AbstractString,
    query_count::Int,
)
    isempty(partitions.confirmation_indices)&&throw(
        ArgumentError("confirmation indices are required when sealing partitions"),
    )
...
function load_query_partitions(
    path::AbstractString;
    dataset_hash::AbstractString,
    ...
    String(data["dataset_hash"])==dataset_hash||throw(
        ArgumentError("query partitions belong to another dataset"),
    )
    confirmation_indices=Int[]

    if allow_confirmation
        sealed_path="$(path).confirmation.toml"
        isfile(sealed_path)||throw(
            ArgumentError("sealed confirmation partition does not exist"),
        )
        sealed=TOML.parsefile(sealed_path)
        String(sealed["dataset_hash"])==dataset_hash||throw(
            ArgumentError("confirmation partition belongs to another dataset"),
        )
        String(sealed["confirmation_hash"])==String(data["confirmation_hash"])||throw(
            ArgumentError("confirmation partition hash changed"),
        )
...
function query_partition_indices(
    partitions::RealQueryPartitions,
    partition::Symbol;
    allow_confirmation::Bool = false,
)
    ...
    partition===:confirmation||throw(
        ArgumentError("partition must be development, validation or confirmation"),
    )
    allow_confirmation||throw(ArgumentError("confirmation partition is sealed"))
    isempty(partitions.confirmation_indices)&&throw(
        ArgumentError("confirmation partition was not loaded"),
    )
...
function arxiv_manifest(
    ...
    vector_info.count==expected_count||throw(
        DimensionMismatch("database vector count does not match $(scale) dataset"),
    )
    vector_info.dimension==ARXIV_FANNS_DIMENSION||throw(
        DimensionMismatch("database vector dimension does not match source"),
    )
    query_info.count==ARXIV_FANNS_QUERY_COUNT||throw(
        DimensionMismatch("query vector count does not match source"),
    )
    query_info.dimension==vector_info.dimension||throw(
        DimensionMismatch("query dimension does not match database"),
    )
...
function load_arxiv_fanns_indices(
    path::AbstractString,
    ...
    length(labels)==manifest.query_count||throw(
        DimensionMismatch("query attribute count does not match queries"),
    )
    ...
    all_truth=load_ivecs_indices(
        joinpath(path, "ground_truth_em.ivecs"),
        all_indices;
        k = k,
    )
...
function load_arxiv_fanns(
    path::AbstractString;
    scale::Symbol = :medium,
    train_count::Int = 24,
    heldout_count::Int = 48,
    k::Int = 10,
    seed::Int = 42,
    verify_hashes::Bool = false,
)
    ...
    return load_arxiv_fanns_indices(
        path,
        split.train_indices,
...
function verify_arxiv_groundtruth(
    ...
    k>0||throw(ArgumentError("k must be positive"))
    queries=split===:train ? dataset.train_queries :
            split===:heldout ? dataset.heldout_queries :
            throw(ArgumentError("split must be train or heldout"))
    ...
    exact=compute_groundtruth(
        dataset.vectors,
        dataset.metadata,
        queries[:, 1:count];
        k = k,
        metric = :cosine,
        filters = filters[1:count],
    )
    recalls=[recall_at_k(exact[index], truth[index], k) for index = 1:count]
