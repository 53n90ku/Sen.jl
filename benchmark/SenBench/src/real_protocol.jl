function select_real_method(spec::ExperimentSpec,evaluations::AbstractDict,method::Symbol)
    ...
    base=isempty(eligible) ? first(sort(points;by=point->(-point.recall,point.nprobe,point.candidates,point.p95_ms),)) : first(sort(eligible;by=point->(point.nprobe,point.candidates,point.p95_ms),))
    safety_nprobe=min(maximum(point.nprobe for point in points),ceil(Int,base.nprobe*spec.probe_safety_factor))
    safety=[point for point in points if point.nprobe>=safety_nprobe]
    selected=first(sort(safety;by=point->(point.nprobe,-point.recall,point.candidates,point.p95_ms),))
    return(
        method=method,
...
function real_experiment_spec(dataset::ArxivFANNSDataset;name::AbstractString="arxiv-fanns-100k",nlists::Int=64,nprobes::AbstractVector{<:Integer}=[1,2,4,8,16,32,64],k::Int=10,target_recall::Float64=0.95,selection_margin::Float64=0.02,probe_safety_factor::Float64=2.0,repetitions::Int=3,iterations::Int=8,restarts::Int=2,training_count::Int=min(size(dataset.vectors,2),20_000),candidate_multiplier::Float64=16.0,postfilter_safety_factor::Float64=2.0,vector_weight::Float64=0.5,filter_weight::Float64=0.5,rerank_factor::Int=4,seed::Int=dataset.manifest.sampling_seed,)
    selectivities=dataset_selectivities(dataset.metadata,vcat(dataset.train_filters,dataset.heldout_filters))
...
function run_real_benchmark(dataset::ArxivFANNSDataset;name::AbstractString="arxiv-fanns-100k",nlists::Int=64,nprobes::AbstractVector{<:Integer}=[1,2,4,8,16,32,64],k::Int=10,target_recall::Float64=0.95,selection_margin::Float64=0.02,probe_safety_factor::Float64=2.0,repetitions::Int=3,iterations::Int=8,restarts::Int=2,training_count::Int=min(size(dataset.vectors,2),20_000),candidate_multiplier::Float64=16.0,postfilter_safety_factor::Float64=2.0,vector_weight::Float64=0.5,filter_weight::Float64=0.5,rerank_factor::Int=4,seed::Int=dataset.manifest.sampling_seed,)
    ...
    context=nothing
    build_seconds=@elapsed context=build_benchmark_context(dataset.vectors,dataset.metadata;nlists=spec.nlists,iterations=spec.iterations,seed=spec.seed,metric=spec.metric,restarts=spec.restarts,training_count=spec.training_count,)
    ...
    for method in (:ivf_prefilter,:filter_aware,:filter_aware_bound)
        ...
        for nprobe in spec.nprobes
            method_evaluations[nprobe]=evaluate_benchmark_method(spec,context,dataset.vectors,dataset.metadata,dataset.train_queries,dataset.train_filters,dataset.train_truth,method,nprobe;split=:train,)
        end
        train_evaluations[method]=method_evaluations
    end

    selections=[select_real_method(spec,train_evaluations[method],method) for method in (:ivf_prefilter,:filter_aware,:filter_aware_bound)]
    ranking_seconds=@elapsed rankings=build_postfilter_rankings(spec,context.index.ivf,dataset.vectors,dataset.train_queries)
    postfilter_selection=select_postfilter_configuration(spec,context,dataset.vectors,dataset.metadata,dataset.train_queries,dataset.train_filters,dataset.train_truth;prebuilt_rankings=rankings,)
    ...
    push!(selections,(
        ...
        train_p50_ms=NaN,
        train_p95_ms=NaN,
        train_candidates=sum(count_exact_candidates(context.filter_index,filter) for filter in dataset.train_filters)/length(dataset.train_filters),
        achieved=true,
        selection_target=spec.target_recall,
        postfilter_candidate_multiplier=0.0,
    ))
    sort!(selections;by=selection->findfirst(==(selection.method),experiment_methods(spec)),)
    ...
    for selection in selections
        result=evaluate_benchmark_method(spec,context,dataset.vectors,dataset.metadata,dataset.heldout_queries,dataset.heldout_filters,dataset.heldout_truth,selection.method,selection.nprobe;split=:heldout,postfilter_candidate_multiplier=selection.postfilter_candidate_multiplier,)
        method_results[selection.method]=result
    end

    auto_result=evaluate_benchmark_method(spec,context,dataset.vectors,dataset.metadata,dataset.heldout_queries,dataset.heldout_filters,dataset.heldout_truth,:auto,0;split=:heldout,db=db,planner_config=planner_config,)
    ...
    for method in vcat(experiment_methods(spec),[:auto])
        for row in method_results[method].rows
            ...
            push!(raw_rows,merge((dataset=dataset.manifest.name,source_query_index=dataset.heldout_indices[query_index],query_selectivity=selectivities[query_index],),row))
        end
    end

    eligible=[(method=method,evaluation=method_results[method],) for method in experiment_methods(spec) if method_results[method].summary.average_recall>=spec.target_recall]
    oracle=isempty(eligible) ? nothing : first(sort(eligible;by=value->value.evaluation.summary.latency.p95_ms,))
    planner_regret=oracle===nothing ? Inf : auto_result.summary.latency.p95_ms/oracle.evaluation.summary.latency.p95_ms
    approximate_methods=filter(!=(:exact),experiment_methods(spec))
    best_approximate_recall=maximum(method_results[method].summary.average_recall for method in approximate_methods)
...
function save_real_benchmark(path::AbstractString,result)
    ...
    dataset_manifest=save_dataset_manifest(joinpath(path,"dataset.toml"),result.dataset_manifest)
    ...
    return(raw=raw,summary=summary,dataset=dataset_manifest,environment=environment_path,)
end
