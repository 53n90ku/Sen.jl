...
function benchmark_summary(
    ...
    result_counts::Vector{Int},
    visited_counts::Vector{Int},
    scored_counts::Vector{Int},
    probe_counts::Vector{Int},
    times_ns::Vector{Int},
)
    return (
...
function run_benchmark(
    ...
    repetitions::Int = 3,
    metric::Symbol = :cosine,
    bound_minimum_nprobe::Int = 1,
    bound_max_nprobe::Int = nprobe,
    ...
    context.filter_index.count==vector_count||throw(
        DimensionMismatch("benchmark context doesnt match vectors"),
    )
    context.index.ivf.metric===metric||throw(
        ArgumentError("benchmark metric doesnt match index metric"),
    )
    ...
    for query_index = 1:query_count
        ...
        if bound_enabled
            bound_measurement=measure_latency(
                ()->search_filter_aware_bound_with_stats(
                    context.index,
                    vectors,
                    metadata,
                    query;
                    filter = filter,
                    k = k,
                    minimum_nprobe = bound_minimum_nprobe,
                    max_nprobe = bound_max_nprobe,
                    metric = metric,
                );
                repetitions = repetitions,
            )
        ...
        aware_work=count_filter_aware_work(
            context.index,
            metadata,
            query,
            filter;
    ...
    return (
        ...
        ivf_prefilter = benchmark_summary(
            prefilter_recalls,
            prefilter_counts,
            prefilter_visited,
            prefilter_scored,
        ...
        ivf_postfilter = benchmark_summary(
            postfilter_recalls,
            postfilter_counts,
            postfilter_visited,
            postfilter_scored,
        ...
        filter_aware = benchmark_summary(
            aware_recalls,
            aware_counts,
            aware_visited,
            aware_scored,
        ...
        filter_aware_bound = bound_enabled ?
                             benchmark_summary(
            bound_recalls,
            bound_counts,
            bound_visited,
