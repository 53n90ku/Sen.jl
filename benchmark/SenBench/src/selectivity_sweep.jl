...
function generate_selectivity_metadata(order::AbstractVector{<:Integer},selectivity::Float64)
    count=length(order)
    ...
    sort(Int.(order))==collect(1:count)||throw(ArgumentError("order must contain every index once"))

    matching_count=round(Int,count*selectivity)
    selected=falses(count)
    selected[order[1:matching_count]].=true

    return[(selected=selected[index],) for index in 1:count]
end
...
function run_selectivity_sweep(vectors::AbstractMatrix,queries::AbstractMatrix,selectivities::AbstractVector{<:Real};nlists::Int,k::Int=10,nprobe::Int=1,iterations::Int=20,seed::Int=42,repetitions::Int=3,metric::Symbol=:cosine,postfilter_oversample::Int=10,adaptive_postfilter::Bool=true,vector_weight::Float64=0.5,filter_weight::Float64=0.5,adaptive::Bool=false,max_nprobe::Int=nprobe,candidate_multiplier::Float64=4.0,rerank_factor::Int=4,)
    ...
    ivf=build_ivf(vectors;nlists=nlists,iterations=iterations,seed=seed,metric=metric,)
