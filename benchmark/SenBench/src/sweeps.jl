function run_nprobe_sweep(context::BenchmarkContext,vectors::AbstractMatrix,metadata::AbstractVector,queries::AbstractMatrix,filters::AbstractVector,nprobes::AbstractVector{<:Integer};k::Int=10,repetitions::Int=3,metric::Symbol=:cosine,postfilter_oversample::Int=10,adaptive_postfilter::Bool=true,adaptive::Bool=false,max_nprobe::Union{Nothing,Int}=nothing,candidate_multiplier::Float64=4.0,vector_weight::Float64=0.5,filter_weight::Float64=0.5,rerank_factor::Int=4,)
    ...
    all(probe->1<=probe<=list_count,probe_values)||throw(ArgumentError("nprobe must be between 1 and list count"))
    resolved_max_nprobe=max_nprobe===nothing ? maximum(probe_values) : max_nprobe
    1<=resolved_max_nprobe<=list_count||throw(ArgumentError("max nprobe must be between 1 and list count"))
...
function run_nprobe_sweep(vectors::AbstractMatrix,metadata::AbstractVector,queries::AbstractMatrix,filters::AbstractVector,nprobes::AbstractVector{<:Integer};nlists::Int,k::Int=10,iterations::Int=20,seed::Int=42,repetitions::Int=3,metric::Symbol=:cosine,restarts::Int=1,training_count::Int=size(vectors,2),postfilter_oversample::Int=10,adaptive_postfilter::Bool=true,adaptive::Bool=false,max_nprobe::Union{Nothing,Int}=nothing,candidate_multiplier::Float64=4.0,vector_weight::Float64=0.5,filter_weight::Float64=0.5,rerank_factor::Int=4,)
...
function best_at_recall(sweep_results::AbstractVector,method::Symbol,target_recall::Float64)
    0.0<=target_recall<=1.0||throw(ArgumentError("target recall must be between 0 and 1"))
    eligible=[point for point in sweep_points(sweep_results,method) if point.recall>=target_recall]
...
function conservative_at_recall(sweep_results::AbstractVector,method::Symbol,target_recall::Float64;probe_safety_factor::Float64=2.0,)
...
function compare_at_recall(sweep_results::AbstractVector,methods::AbstractVector{Symbol},target_recall::Float64)
    return[(method=method,result=best_at_recall(sweep_results,method,target_recall),) for method in methods]
end
