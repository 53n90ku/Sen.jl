function measure_vector_load(
    ...
    for repetition = 1:repetitions
        measurement=@timed load_vector_store(
            path;
            mmap = mmap,
            mmap_threshold_bytes = mmap_threshold_bytes,
        )
...
function run_vector_load_benchmark(
    path::AbstractString;
    repetitions::Int = 5,
    ...
    copied=measure_vector_load(
        path,
        false;
        repetitions = repetitions,
        mmap_threshold_bytes = mmap_threshold_bytes,
    )
    mapped=measure_vector_load(
        path,
        true;
        repetitions = repetitions,
        mmap_threshold_bytes = mmap_threshold_bytes,
    ...
    return (
        ...
        allocation_reduction = 1-mapped.mean_allocated_bytes/max(
            1,
            copied.mean_allocated_bytes,
        ),
    )
end
