...
function generate_filter_order(
    ...
    workload in (:random, :correlated, :anticorrelated, :skewed)||throw(
        ArgumentError("workload must be random, correlated, anticorrelated or skewed"),
    )
    ...
    scores=[skew*projections[index]+randn(rng) for index = 1:count]
    return collect(sortperm(scores; rev = true))
end

function generate_filter_metadata(
    vectors::AbstractMatrix,
    selectivity::Float64;
    workload::Symbol = :random,
    seed::Int = 42,
...
function generate_expression_metadata(
    vectors::AbstractMatrix;
    workload::Symbol = :random,
    seed::Int = 42,
    skew::Float64 = 4.0,
...
function expression_bucket_selectivity(bucket::Symbol)
    bucket in keys(EXPRESSION_BUCKETS)||throw(
        ArgumentError("unknown expression selectivity bucket"),
    )
...
function build_expression_filter(family::Symbol, bucket::Symbol, count::Int)
    ...
    family===:range_date&&return Range(
        :created,
        Date(2020, 1, 1),
        Date(2020, 1, 1)+Day(target_count-1),
    )
...
function build_expression_workload_case(
    ...
    workload in (:random, :correlated, :anticorrelated, :skewed, :natural)||throw(
        ArgumentError("unknown expression workload"),
    )
    family in EXPRESSION_FAMILIES||throw(ArgumentError("unknown expression family"))
...
function generate_expression_workload(
    vectors::AbstractMatrix;
    workload::Symbol = :random,
    seed::Int = 42,
    skew::Float64 = 4.0,
    families = EXPRESSION_FAMILIES,
    buckets = keys(EXPRESSION_BUCKETS),
)
    metadata=generate_expression_metadata(
        vectors;
        workload = workload,
        seed = seed,
        skew = skew,
    ...
    for bucket in resolved_buckets
        for family in resolved_families
            ...
            push!(
                cases,
                build_expression_workload_case(
                    metadata,
                    filter;
                    name = name,
                    family = family,
