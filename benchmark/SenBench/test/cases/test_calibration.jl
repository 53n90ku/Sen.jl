...
function calibration_sweep_result(
    nprobe::Int,
    prefilter_recall::Float64,
    postfilter_recall::Float64,
    aware_recall::Float64,
)
    ...
    return (
        ...
        benchmark = (
            exact = exact,
            ivf_prefilter = prefilter,
            ivf_postfilter = postfilter,
            filter_aware = aware,
            average_selectivity = 0.05,
        ),
    )
end
...
@testset "recall calibration" begin
    ...
    prefilter=lookup_calibrated_nprobe(
        calibration,
        :ivf_prefilter;
        workload = :random,
        selectivity = 0.05,
    ...
    aware=lookup_calibrated_nprobe(
        calibration,
        :filter_aware;
        workload = :random,
        selectivity = 0.05,
    ...
    bound_calibration=calibrate_recall(
        sweep_results;
        workload = :random,
        selectivity = 0.05,
        vector_count = 1000,
        list_count = 4,
        target_recalls = [0.90],
        methods = [:filter_aware_bound],
    )
    @test isempty(bound_calibration.entries)

    conservative=calibrate_recall(
        sweep_results;
        workload = :random,
        selectivity = 0.05,
        vector_count = 1000,
        ...
        target_recalls = [0.90],
        methods = [:ivf_prefilter],
    )
    @test conservative.entries[1].nprobe==4
    @test lookup_calibrated_nprobe(
        conservative,
        :ivf_prefilter;
        workload = :random,
        selectivity = 0.05,
    ...
    safety_factor=calibrate_recall(
        sweep_results;
        workload = :random,
        selectivity = 0.05,
        vector_count = 1000,
        list_count = 4,
        probe_safety_factor = 2.0,
        target_recalls = [0.90],
        methods = [:ivf_prefilter],
    )
...
@testset "heldout calibration evaluation" begin
    ...
    evaluation=evaluate_recall_calibration(
        calibration,
        context,
        vectors,
        metadata,
