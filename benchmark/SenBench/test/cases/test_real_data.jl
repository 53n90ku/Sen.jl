...
@testset "real metadata and splits" begin
    mktempdir() do path
        ...
        open(metadata_path, "w") do io
            println(
                io,
                "{\"main_categories\":[\"cs\"],\"number_of_sub_categories\":1,\"has_comments\":true,\"number_of_versions\":2,\"number_of_authors\":3,\"license\":null}",
            )
            println(
                io,
                "{\"main_categories\":[\"math\"],\"number_of_sub_categories\":2,\"has_comments\":false,\"number_of_versions\":1,\"number_of_authors\":1,\"license\":\"cc-by-4.0\"}",
            )
        end
        ...
        @test split==stratified_query_split(
            labels;
            train_count = 3,
            heldout_count = 3,
            seed = 42,
...
@testset "sealed real query partitions" begin
    ...
    partitions=stratified_query_partitions(
        labels;
        development_count = 6,
        validation_count = 4,
    ...
    @test isempty(
        intersect(
            Set(vcat(partitions.development_indices, partitions.validation_indices)),
            Set(partitions.confirmation_indices),
        ),
    )
    @test partitions==stratified_query_partitions(
        labels;
        development_count = 6,
        validation_count = 4,
        confirmation_count = 4,
    ...
    balanced=stratified_query_partitions(
        labels;
        development_count = 6,
        validation_count = 4,
        confirmation_count = 4,
        seed = 42,
        strata = repeat(1:2, inner = 8),
    )
    ...
    jointly_balanced=balanced_query_partitions(
        labels;
        development_count = 6,
        validation_count = 4,
        confirmation_count = 4,
        seed = 42,
        strata = scarce_strata,
    )
    ...
    mktempdir() do path
        ...
        saved=save_query_partitions(
            base,
            partitions;
            dataset_hash = "dataset",
            query_count = length(labels),
        )
...
@testset "IVF quality diagnostics" begin
    ...
    queries=vectors[:, [1, 4]]
    truth=[[1, 2], [4, 5]]
    metadata=NamedTuple[(label = index<=3 ? 1 : 2,) for index = 1:6]
...
@testset "resumable IVF quality sweep" begin
    ...
    SenBench.normalize_columns!(vectors)
    metadata=NamedTuple[(label = index<=4 ? 1 : 2,) for index = 1:8]
    ...
    train_truth=compute_groundtruth(
        vectors,
        metadata,
        train_queries;
        k = 2,
    ...
    heldout_truth=compute_groundtruth(
        vectors,
        metadata,
        heldout_queries;
        k = 2,
        filters = heldout_filters,
    )
    ...
    mktempdir() do path
        first_result=run_ivf_quality_configuration(
            dataset,
            path;
            nlists = 2,
            training_count = 8,
        ...
        second_result=run_ivf_quality_configuration(
            dataset,
            path;
            nlists = 2,
            training_count = 8,
        ...
        validation=evaluate_hybrid_validation(
            dataset,
            stored_index;
            nprobe = 2,
            k = 2,
...
@testset "groundtruth cache" begin
    mktempdir() do path
        ...
        @test_throws ArgumentError load_groundtruth_cache(
            cache_path;
...
@testset "blockwise exact groundtruth" begin
    ...
    actual=compute_blockwise_groundtruth(
        vectors,
        metadata,
        queries;
        k = 2,
...
@testset "real benchmark protocol" begin
    ...
    @test Set(keys(result.method_results))==Set([
        :exact,
        :ivf_prefilter,
        :ivf_postfilter,
        :filter_aware,
