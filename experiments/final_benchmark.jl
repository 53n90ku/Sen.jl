...
function benchmark_spec(
    ...
    restarts::Int,
    repetitions::Int,
    probe_safety_factor::Float64 = 8.0,
...
function profile_specs(profile::String)
    ...
    elseif profile=="scale"
        ...
        error(
            "profile must be pilot, claim, claim100k, confirm100k, final100k, boundary, dimension100k, scale1m, discovery, confirmation or scale",
        )
    end
...
function main()
    ...
    output_path=get(
        ENV,
        "SEN_OUTPUT",
        joinpath(@__DIR__, "..", "results", "$(profile)-$(round(Int,time()))"),
    )
    ...
    for spec in specs
        ...
        if active_key!=key
            ...
            active_ivf=nothing
            active_build_seconds=@elapsed active_ivf=build_ivf(
                active_base.vectors;
                nlists = spec.nlists,
            ...
            active_postfilter_ranking_seconds=@elapsed active_postfilter_rankings=build_postfilter_rankings(
                spec,
                active_ivf,
                active_base.vectors,
                active_base.train_queries,
            )
            active_key=key
        end
        ...
        result=run_claim_benchmark(
            spec;
            base = active_base,
            prebuilt_ivf = active_ivf,
            prebuilt_build_seconds = active_build_seconds,
            prebuilt_postfilter_rankings = active_postfilter_rankings,
            prebuilt_postfilter_ranking_seconds = active_postfilter_ranking_seconds,
        )
        push!(results, result)
        println(
            "$(spec.name)\t$(result.oracle_method)\t$(round(result.planner_regret;digits=4,))\t$(result.claim.passed)\t$(round(result.claim.speedup_p95;digits=4,))",
        )
        GC.gc()
    end
