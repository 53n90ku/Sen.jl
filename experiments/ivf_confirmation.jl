...
function main()
    get(ENV,"SEN_CONFIRM","0")=="1"||error("confirmation is sealed; set SEN_CONFIRM=1 only after the finalist is frozen")
    ...
    isfile(started_path)&&error("confirmation was already opened; inspect the existing run before doing anything else")
    finalist=TOML.parsefile(finalist_path)
    seed=Int(finalist["seed"])
    manifest=SenBench.arxiv_manifest(data_path,scale,seed)
    String(finalist["dataset_hash"])==manifest.preprocessing_hash||error("finalist dataset changed")
    partitions=load_query_partitions(partition_path;dataset_hash=manifest.preprocessing_hash,allow_confirmation=true,)
    partitions.confirmation_hash==String(finalist["confirmation_hash"])||error("sealed confirmation partition changed")
    ...
    dataset=load_arxiv_fanns_indices(data_path,development,confirmation;scale=scale,k=integer_env("SEN_REAL_K",10),seed=seed,)
    ...
    bucket_gate=all(summary->summary.average_recall>=Float64(finalist["target_recall"]),values(result.buckets))
    ...
    println("confirmation recall=$(round(complete["recall"];digits=4,)) ci95=[$(round(complete["recall_ci_lower"];digits=4,)),$(round(complete["recall_ci_upper"];digits=4,))]")
    println("confirmation exact_p95_ms=$(round(complete["exact_p95_ms"];digits=4,)) hybrid_p95_ms=$(round(complete["hybrid_p95_ms"];digits=4,)) speedup=$(round(complete["speedup_p95"];digits=4,))")
