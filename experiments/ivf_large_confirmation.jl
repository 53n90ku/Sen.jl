...
function finalist_candidate(path::String)
    ...
    for name in readdir(path)
        ...
        isfile(joinpath(candidate, "validation", "summary.tsv"))&&push!(
            candidates,
            candidate,
        )
    end
...
function print_confirmation(result)
    ...
    println(
        "confirmation recall=$(round(summary.average_recall;digits=6,)) ci=[$(round(interval.lower;digits=6,)),$(round(interval.upper;digits=6,))] p95_ms=$(round(summary.latency.p95_ms;digits=4,)) speedup=$(round(result.speedup_p95;digits=4,)) candidates=$(round(summary.average_candidates_scored;digits=2,)) gate=$(result.recall_gate)",
    )
    for (bucket, bucket_summary) in
        sort!(collect(result.buckets); by = value->string(first(value)))
        println(
            "confirmation bucket=$(bucket) recall=$(round(bucket_summary.average_recall;digits=6,))",
        )
    end
end
...
function main()
    ...
    public_partitions=load_query_partitions(
        partition_path;
        dataset_hash = manifest.preprocessing_hash,
    )
    ...
    isfile(joinpath(confirmation_path, "summary.tsv"))&&error(
        "confirmation already completed",
    )
    ...
    partitions.confirmation_hash==String(frozen["confirmation_hash"])||error(
        "sealed confirmation hash changed",
    )
    ...
    println(
        "opening sealed confirmation queries=$(length(confirmation)) frozen=$(basename(candidate))",
    )
    ...
    index_file=joinpath(candidate, "index", "index.bin")
    file_sha256(index_file)==String(frozen["index_sha256"])||error(
