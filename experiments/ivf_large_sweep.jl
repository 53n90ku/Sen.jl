...
function main()
    ...
    partition_name=rehearsal ? "sen_query_partitions_rehearsal_$(vector_count)" : "sen_query_partitions_300k_v1"
    ...
    manifest.vector_count==vector_count||error("prepared dataset vector count does not match SEN_SUBSET_COUNT")
    ...
    println("loading groundtruth development=$(length(development)) validation=$(length(validation)) confirmation=sealed")
    ...
    results=run_ivf_quality_sweep(
        dataset,
        output_path;
        nlists_values=integer_list_env("SEN_SWEEP_NLISTS",rehearsal ? [64,128] : [1_024,1_536,2_048]),
        training_count=integer_env("SEN_SWEEP_TRAINING_COUNT",rehearsal ? min(20_000,vector_count) : 50_000),
