...
function print_result(name::String,result)
    ...
    println("$(name) recall=$(round(summary.average_recall;digits=6,)) p95_ms=$(round(summary.latency.p95_ms;digits=4,)) speedup=$(round(result.speedup_p95;digits=4,)) candidates=$(round(summary.average_candidates_scored;digits=2,)) gate=$(result.recall_gate)")
    for(bucket,bucket_summary) in sort!(collect(result.buckets);by=value->string(first(value)),)
        println("$(name) bucket=$(bucket) recall=$(round(bucket_summary.average_recall;digits=6,))")
    end
end
...
function main()
    ...
    dataset=load_arxiv_subset(data_path,development,validation;k=integer_env("SEN_REAL_K",10),)
    results=NamedTuple[]
