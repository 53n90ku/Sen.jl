...
function freeze_finalist(path::String,checkpoint::Dict,index_file::String,partitions::RealQueryPartitions,dataset_hash::String;nprobe::Int,rare_threshold::Float64,)
    data=Dict(
...
function print_validation(result)
    ...
    for(method,evaluation) in ((:exact,result.exact),(:ivf_prefilter,result.approximate),(:hybrid,result.hybrid))
        ...
        println("$(method)\t$(round(summary.average_recall;digits=4,))\t$(round(summary.latency.p50_ms;digits=4,))\t$(round(summary.latency.p95_ms;digits=4,))\t$(round(summary.average_candidates_scored;digits=2,))")
    end
    for(bucket,summary) in sort!(collect(result.buckets);by=value->string(first(value)),)
        println("hybrid_$(bucket)\t$(round(summary.average_recall;digits=4,))\t$(round(summary.latency.p50_ms;digits=4,))\t$(round(summary.latency.p95_ms;digits=4,))\t$(round(summary.average_candidates_scored;digits=2,))")
    end
end
...
function main()
    ...
    sweep_path=get(ENV,"SEN_SWEEP_OUTPUT",joinpath(root,"results","ivf-quality-$(scale)"))
    champion_path=get(ENV,"SEN_FINALIST_INDEX",joinpath(sweep_path,"lists-512-train-50000-iter-16-restart-3-seed-42"))
    ...
    if !isfile("$(partition_path).toml")
        ...
        created=balanced_query_partitions(labels;development_count=256,validation_count=128,confirmation_count=256,seed=seed,strata=strata,)
        save_query_partitions(partition_path,created;dataset_hash=manifest.preprocessing_hash,query_count=manifest.query_count,)
    ...
    dataset=load_arxiv_fanns_indices(data_path,development,validation;scale=scale,k=k,seed=seed,)
    checkpoint=TOML.parsefile(joinpath(champion_path,"checkpoint.toml"))
    index_file=joinpath(champion_path,"index","index.bin")
    finalist=freeze_finalist(joinpath(sweep_path,"finalist_v3.toml"),checkpoint,index_file,partitions,manifest.preprocessing_hash;nprobe=nprobe,rare_threshold=rare_threshold,)
    index=SenBench.load_ivf_index(joinpath(champion_path,"index"))
    file_sha256(index_file)==String(finalist["index_sha256"])||error("frozen finalist index changed")
    ...
    println("validation_recall_gate=$(result.recall_gate) speedup_p95=$(round(result.speedup_p95;digits=4,))")
