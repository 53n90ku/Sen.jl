...
function main()
    ...
    partition_name=vector_count<300_000 ? "sen_query_partitions_rehearsal_$(vector_count)" :
    ...
    if !isfile("$(partition_path).toml")
        ...
        strata=Int[
            selectivity_bucket(value)===:rare ? 1 :
            selectivity_bucket(value)===:medium ? 2 : 3 for value in selectivities
        ]
    ...
    println(
        "prepared vectors=$(manifest.vector_count) development=$(length(development)) validation=$(length(validation)) confirmation=sealed",
    )
