...
function print_summary(result)
    ...
    for row in real_benchmark_summary_rows(result)
        println(
            "$(row.method)\t$(round(row.heldout_recall;digits=4,))\t$(round(row.p50_ms;digits=4,))\t$(round(row.p95_ms;digits=4,))\t$(round(row.qps;digits=2,))\t$(round(row.candidates_scored;digits=2,))",
        )
    end
end
...
function main()
    ...
    output_path=get(
        ENV,
        "SEN_OUTPUT",
        joinpath(root, "results", "real-$(scale)-$(round(Int,time()))"),
    )
    ...
    verification.passed||error(
        "published groundtruth verification failed recalls=$(verification.recalls)",
    )
    action=="prepare"&&return dataset

    println(
        "running real benchmark vectors=$(size(dataset.vectors,2)) dimension=$(size(dataset.vectors,1))",
    )
    ...
    println(
        "recall_gate=$(result.recall_gate) planner_gate=$(result.planner_gate) passed=$(result.passed)",
    )
    println(
        "oracle=$(result.oracle_method) planner_regret=$(round(result.planner_regret;digits=4,))",
    )
