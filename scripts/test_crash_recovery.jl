...
function wait_for_crash_marker(
...
function kill_at_database_boundary(
    path::AbstractString,
    stage::Symbol,
    mode::Symbol,
    ...
    process=run(
        pipeline(ignorestatus(command), stdout = stdout_io, stderr = stderr_io);
        wait = false,
    )

    try
        marker_contents=wait_for_crash_marker(process, marker, stderr_path)
        occursin("stage=$(stage)", marker_contents)||error(
            "crash worker reached the wrong boundary",
        )
...
function verify_recovered_database(
    path::AbstractString,
    record_id::AbstractString,
    expected_revision::UInt64;
    present::Bool,
    expected_source::AbstractString = "crash-worker",
)
    db=load_db(path)
...
function exercise_storage_fault(stage::Symbol)
    mktempdir() do root
        ...
        if stage===:wal_append
            ...
            @test_throws Sen.InjectedDatabaseStorageError insert!(
                db,
                [0.0, 1.0],
                (source = "fault",);
                id = "fault",
