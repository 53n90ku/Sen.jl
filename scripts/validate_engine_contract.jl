...
function required_string(table::AbstractDict, key::String, context::String)
    ...
    require_contract(
        value isa String&&!isempty(strip(value)),
...
function validate_policy(contract::AbstractDict)
    ...
    require_contract(
        statuses isa AbstractVector,
        "contract policy allowed_statuses must be an array",
    )
    require_contract(
        Set(String.(statuses))==ALLOWED_STATUSES,
        "contract policy allowed_statuses must exactly match the validator",
    )
    require_contract(
        get(policy, "engine_claim_requires_all_gates", nothing)===true,
        "engine claim must require every gate",
    )
...
function validate_gate(root::AbstractString, gate::AbstractDict, index::Int)
    ...
    require_contract(
        ...
        "$(context) has invalid status $(repr(status))",
    )
    stage=get(gate, "stage", nothing)
    require_contract(
        stage isa Integer&&stage>=0,
        "$(context) stage must be a nonnegative integer",
    )
    evidence=get(gate, "evidence", nothing)
    require_contract(
        evidence isa AbstractVector&&!isempty(evidence),
        "$(context) must have evidence",
    )
    evidence_paths=String[
        repository_evidence_path(
            root,
            required_string(Dict("path"=>path), "path", "$(context) evidence"),
    ...
    if status=="pass"
        ...
        require_contract(
            !startswith(lowercase(verification), "pending"),
            "$(context) cannot pass with pending verification",
        )
    end
...
function load_contract(
    root::AbstractString;
    path::AbstractString = joinpath(root, "engine_contract.toml"),
)
    isfile(path)||throw(ArgumentError("engine contract does not exist: $(path)"))
    contract=TOML.parsefile(path)
    get(contract, "format_version", nothing)==CONTRACT_FORMAT_VERSION||throw(
        ArgumentError("unsupported engine contract format version"),
    )
    ...
    require_contract(
        raw_gates isa AbstractVector,
        "contract gates must be an array of tables",
    )
    gates=NamedTuple[]
    ...
    require_contract(
        Set(ids)==REQUIRED_GATE_IDS,
        "contract gates do not match the frozen Stage 0 gate set",
    )
    return ContractReport(version, scope, claim, gates)
end

engine_ready(report::ContractReport) = all(gate->gate.status=="pass", report.gates)
blocking_gates(report::ContractReport) =
    [gate for gate in report.gates if gate.status!="pass"]
