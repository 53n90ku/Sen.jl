...
quiet="--quiet" in ARGS
report=validate_quality_contract(
    contract_path;
    run_benchmarks = !fingerprints,
...
elseif !quiet
    println(
        "Sen quality contract $(report.contract.contract_version), index version $(report.contract.index_version)",
    )

    for workload in report.results
        println(
            "[$(workload.passed ? "PASS" : "FAIL")] $(workload.id) dataset=$(workload.dataset_sha256) workload=$(workload.workload_sha256)",
        )

        for method in workload.methods
            marker=method.recall_passed&&method.latency_passed ? "PASS" : "FAIL"
            println(
                "  [$marker] $(method.method) nprobe=$(method.selected_nprobe) recall=$(round(method.heldout_recall;digits=4)) p95_ms=$(round(method.p95_ms;digits=3))",
            )
        end
    end
end
