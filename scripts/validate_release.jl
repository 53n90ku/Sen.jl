...
require_release(isfile(joinpath(root,"engine_contract.toml")),"engine contract is missing")
require_release(isfile(joinpath(root,"scripts","validate_engine_contract.jl")),"engine contract validator is missing")
require_release(isfile(joinpath(root,"scripts","test_engine_contract.jl")),"engine contract tests are missing")
require_release(isfile(joinpath(root,"quality_contract.toml")),"quality contract is missing")
require_release(isfile(joinpath(root,"benchmark","SenBench","src","quality_contract.jl")),"quality contract runner is missing")
require_release(isfile(joinpath(root,"scripts","validate_quality_contract.jl")),"quality contract validator is missing")
require_release(isfile(joinpath(root,"scripts","test_crash_recovery.jl")),"crash recovery contract tests are missing")
require_release(isfile(joinpath(root,"scripts","crash_worker.jl")),"crash recovery worker is missing")
