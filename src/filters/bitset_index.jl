...
function push_range_value!(builders::Dict{Tuple{Symbol,Symbol,DataType},Any},field::Symbol,value,position::Int)
...
function selection_mask(selection::DenseStoredSelection,count::Int)
    length(selection.mask)==count||throw(DimensionMismatch("selection count doesnt match metadata index"))
...
function equality_filter_mask(index::MetadataIndex,field::Symbol,value)
    ...
    return selection===nothing ? falses(index.count) : selection_mask(selection,index.count)
end

evaluate_filter(index::MetadataIndex,filter::NamedTuple)=evaluate_filter(index,normalize_filter(filter))
evaluate_filter(index::MetadataIndex,::Nothing)=trues(index.count)
evaluate_filter(index::MetadataIndex,filter::Eq)=equality_filter_mask(index,filter.field,filter.value)
...
function add_range_matches!(mask::BitVector,lane::RangeLane,lower,upper)
    ...
    @inbounds for sorted_index in first_index:last_index
...
supports_indexed_filter(index::MetadataIndex,filter::And)=all(child->supports_indexed_filter(index,child),filter.children)
supports_indexed_filter(index::MetadataIndex,filter::Or)=all(child->supports_indexed_filter(index,child),filter.children)
supports_indexed_filter(index::MetadataIndex,filter::Not)=supports_indexed_filter(index,filter.child)
supports_indexed_filter(index::MetadataIndex,filter::NamedTuple)=supports_indexed_filter(index,normalize_filter(filter))
