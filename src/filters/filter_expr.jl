...
function filter_value_isequal(left, right)::Bool
    ...
    (left_temporal||right_temporal)&&return typeof(left)===typeof(right)&&isequal(
        left,
        right,
    )
...
function freeze_in_values(values)
    ...
    try
        for value in values
            value_count+=1
            value_count<=MAX_FILTER_IN_VALUES||throw(
                ArgumentError("In cannot contain more than $(MAX_FILTER_IN_VALUES) values"),
            )
...
function validate_range_bounds(lower, upper)
    ...
    lower_domain===nothing&&throw(
        ArgumentError("Range bounds must be numeric, Date, or DateTime values"),
    )
    lower_domain===upper_domain||throw(
        ArgumentError("Range bounds must use compatible domains"),
    )
    lower isa AbstractFloat&&isnan(lower)&&throw(
        ArgumentError("Range bounds cannot be NaN"),
    )
    upper isa AbstractFloat&&isnan(upper)&&throw(
        ArgumentError("Range bounds cannot be NaN"),
    )
...
struct Range{L,U} <: FilterExpr
    ...
    function Range(field::Symbol, lower, upper)
        ...
        return new{typeof(frozen_lower),typeof(frozen_upper)}(
            field,
            frozen_lower,
            frozen_upper,
        )
...
struct And{T<:Tuple} <: FilterExpr
    ...
    function And(children::Tuple)
        all(child->child isa FilterExpr, children)||throw(
            ArgumentError("And children must be filter expressions"),
        )
...
struct Or{T<:Tuple} <: FilterExpr
    ...
    function Or(children::Tuple)
        all(child->child isa FilterExpr, children)||throw(
            ArgumentError("Or children must be filter expressions"),
        )
        return new{typeof(children)}(children)
    end
end
...
function validate_filter(filter::FilterExpr)
    ...
    while !isempty(pending)
        ...
        depth<=MAX_FILTER_DEPTH||throw(
            ArgumentError("filter depth cannot exceed $(MAX_FILTER_DEPTH)"),
        )
        node_count+=1
        node_count<=MAX_FILTER_NODES||throw(
            ArgumentError("filter cannot contain more than $(MAX_FILTER_NODES) nodes"),
        )
        ...
        elseif current isa In
            length(current.values)<=MAX_FILTER_IN_VALUES||throw(
                ArgumentError("In cannot contain more than $(MAX_FILTER_IN_VALUES) values"),
            )
...
matches_filter(metadata::NamedTuple, filter::And)::Bool =
    all(child->matches_filter(metadata, child), filter.children)
matches_filter(metadata::NamedTuple, filter::Or)::Bool =
    any(child->matches_filter(metadata, child), filter.children)
matches_filter(metadata::NamedTuple, filter::Not)::Bool =
    !matches_filter(metadata, filter.child)
matches_filter(metadata::NamedTuple, filter::NamedTuple)::Bool =
    matches_filter(metadata, normalize_filter(filter))

Base.isequal(::FilterExpr, ::FilterExpr) = false
Base.isequal(left::Eq, right::Eq) =
    left.field===right.field&&filter_value_isequal(left.value, right.value)
...
function Base.isequal(left::In, right::In)
    ...
    return all(
        filter_value_isequal(left.values[index], right.values[index]) for
        index in eachindex(left.values)
    )
end

Base.isequal(left::Range, right::Range) =
    left.field===right.field&&filter_value_isequal(left.lower, right.lower)&&filter_value_isequal(
        left.upper,
        right.upper,
    )
...
Base.hash(filter::Eq, seed::UInt) =
    filter_value_hash(filter.value, hash(filter.field, hash(:Eq, seed)))
...
Base.hash(filter::Range, seed::UInt) = filter_value_hash(
    filter.upper,
    filter_value_hash(filter.lower, hash(filter.field, hash(:Range, seed))),
)
