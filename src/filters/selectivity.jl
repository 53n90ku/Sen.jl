function estimate_selectivity(
    index::MetadataIndex,
    filter::Union{NamedTuple,FilterExpr};
    metadata::Union{Nothing,AbstractVector} = nothing,
)::Float64
    index.count==0&&return 0.0
    metadata===nothing||length(metadata)==index.count||throw(
        DimensionMismatch("metadata count doesnt match metadata index"),
    )
    ...
    matching_count=if supports_indexed_filter(index, expression)
        ...
        metadata===nothing&&throw(
            ArgumentError(
                "filter selectivity requires metadata because this expression is not supported by the metadata index",
            ),
        )
