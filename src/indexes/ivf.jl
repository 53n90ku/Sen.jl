...
IVFIndex(centroids::Matrix{Float32}, lists::Vector{Vector{Int}}) = IVFIndex(
    centroids,
    lists,
    Float32[],
    :cosine,
    :cosine,
    fill(Float32(pi), length(lists)),
    fill(-1.0f0, length(lists)),
    zeros(Float32, length(lists)),
)
IVFIndex(
    centroids::Matrix{Float32},
    lists::Vector{Vector{Int}},
...
function nearest_centroid_score(
    centroids::AbstractMatrix,
    vector::AbstractVector,
    metric::Symbol,
)
    ...
    for list_index = 1:centroid_count
...
function sample_centroid_position(
    rng::AbstractRNG,
...
function initialize_centroids(
    ...
    training_indices::AbstractVector{<:Integer},
    nlists::Int,
    metric::Symbol,
    rng::AbstractRNG,
)
...
function cosine_assignments(centroids::AbstractMatrix, vectors::AbstractMatrix)
    ...
    for vector_index = 1:vector_count
        ...
        @inbounds for list_index = 2:list_count
...
function initialize_cosine_centroids(
    training_vectors::Matrix{Float32},
    nlists::Int,
    rng::AbstractRNG,
    vector_norms::AbstractVector,
)
    ...
    for list_index = 1:nlists
        ...
        @inbounds @simd for position = 1:training_count
            if selected[position]
                minimum_losses[position]=0.0f0
            else
                similarity=scores[position]/vector_norms[position]
                minimum_losses[position]=min(
                    minimum_losses[position],
                    max(0.0f0, 1.0f0-similarity),
                )
            end
        end
...
function train_cosine_centroids(
    vectors::AbstractMatrix,
    training_indices::AbstractVector{<:Integer},
    nlists::Int,
    iterations::Int,
    ...
    any(iszero, vector_norms)&&throw(
        ArgumentError("cosine training vectors cannot be zero"),
    )
    ...
    for _ = 1:restarts
        ...
        for _ = 1:iterations
            ...
            for vector_index = 1:training_count
                list_index=assignments[vector_index]
                list_counts[list_index]+=1
                @inbounds @simd for axis = 1:dimension
            ...
            for list_index = 1:nlists
                ...
                if list_counts[list_index]==0||collapsed
                    new_centroids[:, list_index].=training_vectors[
                        :,
                        rand(rng, 1:training_count),
                    ]
                end
        ...
        loss=sum(
            1.0-nearest.best_scores[index]/vector_norms[index] for index = 1:training_count
        )
        if loss<best_loss
            best_loss=loss
...
function train_centroids(
    ...
    nlists<=training_count<=count||throw(
        ArgumentError("training count must be between nlists and vector count"),
    )

    rng=MersenneTwister(seed)
    training_indices=training_count==count ? collect(1:count) :
                     randperm(rng, count)[1:training_count]

    if metric===:cosine
        return train_cosine_centroids(
            vectors,
            training_indices,
            nlists,
            iterations,
    ...
    for _ = 1:restarts
        ...
        for _ = 1:iterations
            ...
            for (position, vector_index) in enumerate(training_indices)
                ...
                @inbounds @simd for dimension = 1:dim
                    new_centroids[dimension, list_index]+=Float32(
                        vectors[dimension, vector_index],
                    )
                end
            end

            for list_index = 1:nlists
                collapsed=metric===:cosine&&iszero(
                    vector_norm(@view new_centroids[:, list_index]),
                )

                if list_counts[list_index]==0||collapsed
                    random_position=rand(rng, 1:training_count)
                    new_centroids[:, list_index].=vectors[
                        :,
                        training_indices[random_position],
                    ]
                elseif metric===:dot
...
function compute_list_radii(
    ...
    radii=metric===:cosine ? zeros(Float32, length(lists)) :
          fill(Float32(pi), length(lists))
    ...
    for list_index in eachindex(lists)
        ...
        for vector_index in lists[list_index]
            ...
            similarity=clamp(
                column_dot(centroid, vectors, vector_index)/norm,
                -1.0f0,
                1.0f0,
            )
...
function centroid_distances!(
    ...
    index.routing===:cosine&&iszero(query_norm)&&throw(
        ArgumentError("query vector cannot be zero"),
    )

    for list_index = 1:list_count
...
function centroid_distances(index::IVFIndex, query::AbstractVector)
    return centroid_distances!(
        Vector{Float32}(undef, size(index.centroids, 2)),
        index,
        query,
    )
end
...
function list_score_upper_bound(index::IVFIndex, query::AbstractVector, list_index::Int)
    ...
    length(query)==size(index.centroids, 1)||throw(
        DimensionMismatch("query dimension doesnt match centroids"),
    )
...
function list_score_upper_bound(
    index::IVFIndex,
    query::AbstractVector,
    list_index::Int,
    query_norm::Float32,
...
function list_score_upper_bounds(index::IVFIndex, query::AbstractVector)
    ...
    iszero(query_norm)&&throw(ArgumentError("query vector cannot be zero"))
    return [
        list_score_upper_bound(index, query, list_index, query_norm) for
        list_index in eachindex(index.lists)
    ]
end

function rank_ivf_lists!(
...
function collect_ivf_candidates!(
    ...
    sizehint!(
        candidate_indices,
        sum(length(index.lists[list_index]) for list_index in selected_lists),
    )
...
function _filter_ivf_candidates(
    candidate_indices::AbstractVector{<:Integer},
    metadata::AbstractVector,
    filter::FilterExpr;
    filter_index::Union{Nothing,BitsetIndex} = nothing,
)
    filter_index===nothing||filter_index.count==length(metadata)||throw(
        DimensionMismatch("filter index count doesnt match metadata"),
    )

    if filter_index===nothing||!supports_indexed_filter(filter_index, filter)
        return Int[
            index for index in candidate_indices if matches_filter(metadata[index], filter)
        ]
    end
...
function filter_ivf_candidates(
    candidate_indices::AbstractVector{<:Integer},
    metadata::AbstractVector,
    filter::Union{NamedTuple,FilterExpr};
    filter_index::Union{Nothing,BitsetIndex} = nothing,
)
    return _filter_ivf_candidates(
        candidate_indices,
        metadata,
...
@inline function candidate_score(
    vectors::AbstractMatrix,
    query::AbstractVector,
    vector_index::Int,
    metric::Symbol,
    query_norm::Float32,
    vector_norms::Union{Nothing,AbstractVector},
)
    score=column_dot(query, vectors, vector_index)

    if metric===:cosine
        stored_norm=vector_norms===nothing||isempty(vector_norms) ?
                    column_norm(vectors, vector_index) : Float32(vector_norms[vector_index])
...
function score_candidate_stream!(
    workspace::SearchWorkspace,
    vectors::AbstractMatrix,
    query::AbstractVector,
    candidate_indices;
    ...
    metric===:cosine&&iszero(query_norm)&&throw(
        ArgumentError("query vector cannot be zero"),
    )
    order=0
...
function score_contiguous_candidates!(
    workspace::SearchWorkspace,
    vectors::AbstractMatrix,
    query::AbstractVector,
    candidate_indices::AbstractUnitRange{<:Integer};
    k::Int,
    metric::Symbol,
    vector_norms::Union{Nothing,AbstractVector},
    excluded::Union{Nothing,BitVector},
    ...
    for block_start = first_index:SEARCH_SCORE_BLOCK_SIZE:last_index
        ...
        for offset = 1:block_length
            ...
            if metric===:cosine
                stored_norm=vector_norms===nothing||isempty(vector_norms) ?
                            ...
                            Float32(vector_norms[vector_index])
                iszero(stored_norm)&&throw(ArgumentError("stored vector cannot be zero"))
                score/=query_norm*stored_norm
            end
...
function top_candidate_results(metadata::AbstractVector, workspace::SearchWorkspace)
    ...
    return [
        (
            index = workspace.heap_indices[position],
            score = workspace.heap_scores[position],
            metadata = metadata[workspace.heap_indices[position]],
        ) for position in eachindex(workspace.heap_scores)
    ]
end

function ensure_batch_search_workspace!(
    workspace::BatchSearchWorkspace,
    dim::Int,
    query_count::Int,
    k::Int,
)
    if size(workspace.score_block, 1)<SEARCH_SCORE_BLOCK_SIZE||size(
        workspace.score_block,
        2,
    )<SEARCH_QUERY_BLOCK_SIZE
        workspace.score_block=Matrix{Float32}(
            undef,
            SEARCH_SCORE_BLOCK_SIZE,
            SEARCH_QUERY_BLOCK_SIZE,
        )
    end
...
function search_exact_batch(
    ...
    workspace::BatchSearchWorkspace = batch_search_workspace(),
)
    ...
    vector_norms!==nothing&&!isempty(vector_norms)&&length(vector_norms)!=vector_count&&throw(
        DimensionMismatch("vector norm count doesnt match vectors"),
    )
    ensure_batch_search_workspace!(workspace, dim, query_count, k)

    for query_start = 1:SEARCH_QUERY_BLOCK_SIZE:query_count
        query_stop=min(query_count, query_start+SEARCH_QUERY_BLOCK_SIZE-1)
        query_length=query_stop-query_start+1

        for local_query = 1:query_length
            query_index=query_start+local_query-1
            squared_norm=0.0f0

            @inbounds @simd for axis = 1:dim
            ...
            metric===:cosine&&iszero(workspace.query_norms[query_index])&&throw(
                ArgumentError("query vector cannot be zero"),
            )
        end
        ...
        for block_start = 1:SEARCH_SCORE_BLOCK_SIZE:vector_count
            ...
            for local_vector = 1:block_length
                vector_index=block_start+local_vector-1
                excluded!==nothing&&excluded[vector_index]&&continue
                stored_norm=if metric===:cosine
                    resolved=vector_norms===nothing||isempty(vector_norms) ?
                             column_norm(vectors, vector_index) :
                             Float32(vector_norms[vector_index])
                ...
                for local_query = 1:query_length
                    ...
                    metric===:cosine&&(
                        score/=workspace.query_norms[query_index]*stored_norm
                    )
                    add_top_candidate!(
                        workspace.query_workspaces[query_index],
                        vector_index,
                        score,
                        vector_index,
                        k,
    ...
    return [
        top_candidate_results(metadata, workspace.query_workspaces[query_index]) for
        query_index = 1:query_count
    ]
end

function score_ivf_candidates(
    vectors::AbstractMatrix,
    metadata::AbstractVector,
    query::AbstractVector,
    candidate_indices;
    ...
    vector_norms::Union{Nothing,AbstractVector} = nothing,
    excluded::Union{Nothing,BitVector} = nothing,
    workspace::SearchWorkspace = search_workspace(),
)
    k>0||throw(ArgumentError("k must be positive"))
    metric in (:cosine, :dot)||throw(ArgumentError("metric must be cosine or dot"))
    validate_excluded(excluded, size(vectors, 2))
    vector_norms!==nothing&&!isempty(vector_norms)&&length(vector_norms)!=size(vectors, 2)&&throw(
        DimensionMismatch("vector norm count doesnt match vectors"),
    )
    reset_top_candidates!(workspace, k)

    if candidate_indices isa AbstractUnitRange&&eltype(vectors)===Float32&&length(
        candidate_indices,
    )>=64
        score_contiguous_candidates!(
            workspace,
            vectors,
            query,
            candidate_indices;
...
function rank_scored_candidates(
    metadata::AbstractVector,
    candidate_indices::AbstractVector{<:Integer},
    ...
    k>0||throw(ArgumentError("k must be positive"))
    length(candidate_indices)==length(scores)||throw(
        DimensionMismatch("candidate and score counts dont match"),
    )
...
function search_ivf(
    ...
    candidate_indices=collect_ivf_candidates!(
...
function search_ivf_prefilter(
    ...
    visited_indices=collect_ivf_candidates!(
        workspace.candidate_indices,
        index,
        selected_lists,
    )
    candidate_indices=filter_ivf_candidates!(
        workspace.filtered_indices,
        visited_indices,
        metadata,
        normalized_filter;
        filter_index = filter_index,
...
function resolve_postfilter_oversample(
    minimum_oversample::Int,
    selectivity::Float64;
...
function search_ivf_postfilter(
    index::IVFIndex,
    vectors::AbstractMatrix,
    metadata::AbstractVector,
    query::AbstractVector;
