...
function segment_index_build_config(db::VectorDB,count::Int)
    ...
    return IndexBuildConfig(nlists,count;iterations=iterations,seed=seed,restarts=restarts,training_count=count,)
end
...
function build_segment_index(snapshot)
    ...
    filter_index=snapshot.filter_index===nothing ? build_bitset_index(snapshot.metadata) : snapshot.filter_index
    return(index=index,filter_index=filter_index,index_bytes=database_index_bytes(index,filter_index),)
end

function install_segment_index!(db::VectorDB,segment_id::AbstractString,built,config::IndexBuildConfig)
...
function update_segment_index_state!(state::SegmentIndexState;status::Union{Nothing,Symbol}=nothing,current_segment_id=missing,attempts::Union{Nothing,Int}=nothing,error_message=missing,increment_completed::Bool=false,)
    lock(state.lock)
...
function segment_index_worker!(db::VectorDB)
    ...
    try
        while !segment_index_stop_requested(state)
            ...
            update_segment_index_state!(state;status=:building,current_segment_id=snapshot.id,error_message=nothing,)
            installed=false

            for attempt in 1:(snapshot.max_retries+1)
                ...
                try
                    ...
                    segment_index_stop_requested(state)&&return nothing
                    installed=install_segment_index!(db,snapshot.id,built,snapshot.config)
                    installed&&update_segment_index_state!(state;status=:completed,current_segment_id=nothing,error_message=nothing,increment_completed=true,)
                    break
                catch error
                    error isa InterruptException&&rethrow()
                    final_attempt=attempt>snapshot.max_retries
                    update_segment_index_state!(state;status=final_attempt ? :failed : :retrying,error_message=sprint(showerror,error),)
...
function configure_maintenance!(db::VectorDB,config::MaintenanceConfig)
    ...
    config.incremental_indexing ? maybe_schedule_segment_indexing!(db) : stop_segment_indexing!(db)
    return db
end
