...
@inline function top_candidate_better(
    left_score::Float32,
    left_order::Int,
    right_score::Float32,
    right_order::Int,
)
    return isless(right_score, left_score)||(
        isequal(left_score, right_score)&&left_order<right_order
    )
end
...
@inline function add_top_candidate!(
    ...
    if count<k
        ...
        top_candidate_better(
            score,
            order,
            workspace.heap_scores[end],
            workspace.heap_orders[end],
        )||return workspace
    end

    insertion=count

    while insertion>1&&top_candidate_better(
        score,
        order,
        workspace.heap_scores[insertion-1],
        workspace.heap_orders[insertion-1],
    )
...
function top_k(scores::AbstractVector, k::Int)
    ...
    return [
        (index = workspace.heap_indices[position], score = workspace.heap_scores[position])
