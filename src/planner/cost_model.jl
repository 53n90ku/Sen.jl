...
function PlannerConfig(;target_recall::Float64=0.90,candidate_multiplier::Float64=4.0,postfilter_candidate_multiplier::Float64=candidate_multiplier,default_nprobe::Int=4,max_nprobe::Int=64,centroid_cost::Float64=0.05,metadata_check_cost::Float64=0.10,vector_score_cost::Float64=1.0,filter_aware_gain::Float64=0.75,exact_filter_threshold::Float64=0.01,calibration::Union{Nothing,RecallCalibration}=nothing,workload::Symbol=:auto,)
    ...
    postfilter_candidate_multiplier>0||throw(ArgumentError("postfilter candidate multiplier must be positive"))
    default_nprobe>0||throw(ArgumentError("default nprobe must be positive"))
    max_nprobe>=default_nprobe||throw(ArgumentError("max nprobe cannot be smaller than default nprobe"))
    ...
    0.0<=filter_aware_gain<=1.0||throw(ArgumentError("filter aware gain must be between 0 and 1"))
    0.0<=exact_filter_threshold<=1.0||throw(ArgumentError("exact filter threshold must be between 0 and 1"))
    workload in (:auto,:random,:correlated,:anticorrelated,:skewed,:natural)||throw(ArgumentError("invalid workload"))

    return PlannerConfig(target_recall,candidate_multiplier,postfilter_candidate_multiplier,default_nprobe,max_nprobe,centroid_cost,metadata_check_cost,vector_score_cost,filter_aware_gain,exact_filter_threshold,calibration,workload)
end
...
function estimate_required_nprobe(total_count::Int,nlists::Int,selectivity::Float64,k::Int;candidate_multiplier::Float64=4.0,target_recall::Float64=0.90,max_nprobe::Int=nlists,)
...
function logical_filter_stats(db::VectorDB,filter::Union{NamedTuple,FilterExpr})
    ...
    base_mask=if db.filter_index!==nothing&&supports_indexed_filter(db.filter_index,expression)
        evaluate_filter(db.filter_index,expression)
    else
        BitVector(matches_filter(row,expression) for row in base_metadata)
    end

    length(base_mask)==length(db.base_tombstones)||throw(DimensionMismatch("base filter mask doesnt match tombstones"))
    base_live_count=length(base_mask)-count(db.base_tombstones)
    base_matching_count=count(index->base_mask[index]&&!db.base_tombstones[index],eachindex(base_mask))
...
function estimate_filter_concentration(index::FilterAwareIVFIndex,filter::Union{NamedTuple,FilterExpr};metadata::Union{Nothing,AbstractVector}=nothing,excluded::Union{Nothing,BitVector}=nothing,)
    filter=normalize_filter(filter)
    excluded===nothing||length(excluded)==sum(length,index.ivf.lists)||throw(DimensionMismatch("excluded mask doesnt match index"))
    metadata===nothing||length(metadata)==sum(length,index.ivf.lists)||throw(DimensionMismatch("metadata count doesnt match index"))
    matching_counts=Int[]
    ...
    for list_index in eachindex(index.ivf.lists)
        ...
        mask=if supports_indexed_filter(local_index,filter)
            ...
            metadata===nothing&&throw(ArgumentError("filter concentration requires metadata because this expression is not supported by the list metadata index"))
            BitVector(matches_filter(metadata[vector_index],filter) for vector_index in list)
        end

        push!(matching_counts,count(position->mask[position]&&(excluded===nothing||!excluded[list[position]]),eachindex(list)))
    end
...
function estimate_strategy_costs(db::VectorDB,filter::Union{NamedTuple,FilterExpr};k::Int=10,config::PlannerConfig=PlannerConfig(),)
    filter=normalize_filter(filter)
    filter_index=db.filter_index
    filter_index===nothing&&throw(ArgumentError("filter index must be built before planning"))
    ...
    index=db.index
    estimated_nlists=index===nothing ? min(config.max_nprobe,max(1,ceil(Int,sqrt(total_count)))) : length(index.ivf.lists)
    ...
    concentration=index===nothing ? 0.0 : estimate_filter_concentration(index,filter;metadata=stored_metadata(db.metadata_store),excluded=db.base_tombstones,)
    workload=config.workload===:auto ? classify_filter_workload(concentration) : config.workload
    ...
    aware_nprobe=max(1,ceil(Int,uniform_nprobe*(1-config.filter_aware_gain*concentration)))

    prefilter_calibration=config.calibration===nothing ? nothing : lookup_calibrated_nprobe(config.calibration,:ivf_prefilter;workload=workload,selectivity=selectivity,vector_count=total_count,list_count=estimated_nlists,dimension=db.dim,metric=db.metric,target_recall=config.target_recall,)
    postfilter_calibration=config.calibration===nothing ? nothing : lookup_calibrated_nprobe(config.calibration,:ivf_postfilter;workload=workload,selectivity=selectivity,vector_count=total_count,list_count=estimated_nlists,dimension=db.dim,metric=db.metric,target_recall=config.target_recall,)
    aware_calibration=config.calibration===nothing ? nothing : lookup_calibrated_nprobe(config.calibration,:filter_aware;workload=workload,selectivity=selectivity,vector_count=total_count,list_count=estimated_nlists,dimension=db.dim,metric=db.metric,target_recall=config.target_recall,)
    bound_calibration=config.calibration===nothing ? nothing : lookup_calibrated_nprobe(config.calibration,:filter_aware_bound;workload=workload,selectivity=selectivity,vector_count=total_count,list_count=estimated_nlists,dimension=db.dim,metric=db.metric,target_recall=config.target_recall,)

    prefilter_nprobe=prefilter_calibration===nothing ? required_nprobe : prefilter_calibration.nprobe
    postfilter_nprobe=postfilter_calibration===nothing ? required_nprobe : postfilter_calibration.nprobe
    filter_aware_min_nprobe=aware_calibration===nothing ? min(config.default_nprobe,aware_nprobe) : aware_calibration.nprobe
    filter_aware_max_nprobe=aware_calibration===nothing ? max(aware_nprobe,required_nprobe) : aware_calibration.nprobe
    filter_aware_bound_min_nprobe=1
    filter_aware_bound_max_nprobe=bound_calibration===nothing ? required_nprobe : bound_calibration.nprobe
    ...
    prefilter_scored=min(stats.base_matching_count,ceil(Int,prefilter_visited*base_selectivity))
    ...
    bound_visited=min(stats.base_matching_count,ceil(Int,filter_aware_bound_max_nprobe*average_list_size*aware_density))
