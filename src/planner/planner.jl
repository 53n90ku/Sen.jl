...
function planner_config_key(config::PlannerConfig)
    calibration_key=config.calibration===nothing ? UInt(0) : hash(config.calibration.entries)
    return(
...
function choose_strategy(db::VectorDB,filter::Union{Nothing,NamedTuple,FilterExpr};k::Int=10,config::PlannerConfig=PlannerConfig(),)
    ...
    if estimates.matching_count==0
        return QueryPlan(PreFilterExactStrategy(),estimates.selectivity,0,0,0,estimates.exact,estimates,"filter has no matches")
    end

    if estimates.selectivity<=config.exact_filter_threshold
        return QueryPlan(PreFilterExactStrategy(),estimates.selectivity,estimates.matching_count,0,0,estimates.exact,estimates,"rare filter is cheaper and safer with exact search")
    end

    choices=[
        (strategy=PreFilterExactStrategy(),cost=estimates.exact,minimum_nprobe=0,nprobe=0,reason="exact filtering has the lowest estimated work"),
        (strategy=IVFPreFilterStrategy(),cost=estimates.ivf_prefilter,minimum_nprobe=estimates.prefilter_nprobe,nprobe=estimates.prefilter_nprobe,reason="vector-first lists with prefiltering have the lowest estimated work"),
        (strategy=IVFPostFilterStrategy(),cost=estimates.ivf_postfilter,minimum_nprobe=estimates.postfilter_nprobe,nprobe=estimates.postfilter_nprobe,reason="postfiltering has the lowest estimated work"),
        (strategy=FilterAwareIVFStrategy(),cost=estimates.filter_aware,minimum_nprobe=estimates.filter_aware_min_nprobe,nprobe=estimates.filter_aware_max_nprobe,reason="filter concentration makes metadata-aware probing cheapest"),
    ]

    if db.metric===:cosine&&estimates.filter_aware_bound_calibrated
        push!(choices,(strategy=BoundedFilterAwareIVFStrategy(),cost=estimates.filter_aware_bound,minimum_nprobe=estimates.filter_aware_bound_min_nprobe,nprobe=estimates.filter_aware_bound_max_nprobe,reason="measured angular bounds make progressive probing cheapest",))
    end
...
function plan_query(db::VectorDB,filter::Union{Nothing,NamedTuple,FilterExpr};k::Int=10,strategy::Symbol=:auto,config::PlannerConfig=PlannerConfig(),)
    ...
    selected_cost=hasproperty(automatic_plan.costs,selected_name) ? getproperty(automatic_plan.costs,selected_name) : automatic_plan.estimated_cost
