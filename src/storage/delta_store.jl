...
function insert_delta!(store::DeltaStore,vector::AbstractVector{<:Real},metadata::NamedTuple,id)
...
function update_delta!(store::DeltaStore,id;vector::Union{Nothing,AbstractVector{<:Real}}=nothing,metadata::Union{Nothing,NamedTuple}=nothing,)
...
function validate_delta_store(store::DeltaStore)
    ...
    count==length(store.metadata_store)==length(store.id_store)||error("delta stores are misaligned")
