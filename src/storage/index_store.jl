...
function save_ivf_index(
    path::AbstractString,
    index::IVFIndex;
    filename::AbstractString = "index.bin",
)
    mkpath(path)
    index_path=index_file_path(path, filename)
    dim, list_count=size(index.centroids)
    vector_count=sum(length, index.lists)
    index.metric in (:cosine, :dot)||throw(
        ArgumentError("index metric must be cosine or dot"),
    )
    index.routing in (:cosine, :dot, :euclidean)||throw(
        ArgumentError("index routing must be cosine, dot or euclidean"),
    )
    index.metric===:cosine&&index.routing!==:cosine&&throw(
        ArgumentError("cosine index must use cosine routing"),
    )
    index.metric===:dot&&!(index.routing in (:dot, :euclidean))&&throw(
        ArgumentError("dot index routing is invalid"),
    )
    length(index.vector_norms)==vector_count||throw(
        DimensionMismatch("vector norm count doesnt match index"),
    )
    length(index.list_radii)==list_count||throw(
        DimensionMismatch("radius count doesnt match index"),
    )
    ...
    open(index_path, "w") do io
        ...
        write(
            io,
            index.routing===:cosine ? UInt8(1) : index.routing===:dot ? UInt8(2) : UInt8(3),
        )
        write(io, Int64(dim))
...
function load_ivf_index(path::AbstractString; filename::AbstractString = "index.bin")
    ...
    return open(index_path, "r") do io
        ...
        version in (1, IVF_INDEX_VERSION)||throw(
            ArgumentError("unsupported index format version"),
        )
        metric_code=read(io, UInt8)
        metric=metric_code==1 ? :cosine :
               metric_code==2 ? :dot : throw(ArgumentError("invalid stored index metric"))
        routing=if version>=2
            routing_code=read(io, UInt8)
            routing_code==1 ? :cosine :
            routing_code==2 ? :dot :
            routing_code==3 ? :euclidean :
            throw(ArgumentError("invalid stored index routing"))
        else
            metric===:cosine ? :cosine : :euclidean
        end
        metric===:cosine&&routing!==:cosine&&throw(
            ArgumentError("stored cosine index routing is invalid"),
        )
        metric===:dot&&!(routing in (:dot, :euclidean))&&throw(
            ArgumentError("stored dot index routing is invalid"),
        )
        ...
        vector_count>=list_count||throw(
            ArgumentError("stored vector count cannot be smaller than list count"),
        )
        ...
        all(norm->isfinite(norm)&&norm>0, vector_norms)||throw(
            ArgumentError("stored vector norms must be positive and finite"),
        )
        all(radius->isfinite(radius)&&0<=radius<=Float32(pi), list_radii)||throw(
            ArgumentError("stored list radii are invalid"),
        )
        ...
        for list_index = 1:list_count
            ...
            for position = 1:list_size
                vector_index=Int(read(io, Int64))
                1<=vector_index<=vector_count||throw(
                    ArgumentError("stored vector index is out of bounds"),
                )
                seen[vector_index]&&throw(
                    ArgumentError("stored vector index is duplicated"),
                )
