...
function create_database_manifest(
    dim::Int,
    metric::Symbol,
    count::Int;
    format_version::Int = 1,
    nlists::Union{Nothing,Int} = nothing,
    revision::Integer = count,
    index_revision::Union{Nothing,Integer} = nlists===nothing ? nothing : revision,
    iterations::Int = 20,
    seed::Int = 42,
    restarts::Int = 1,
    training_count::Int = count,
    ...
    index_revision===nothing||index_revision>=0||throw(
        ArgumentError("index revision cannot be negative"),
    )
    index_revision===nothing||index_revision<=revision||throw(
        ArgumentError("index revision cannot exceed database revision"),
    )
    index_revision===nothing||nlists!==nothing||throw(
        ArgumentError("index revision requires an index build config"),
    )

    config=nlists===nothing ? nothing :
           IndexBuildConfig(
        nlists,
        count;
    ...
    return DatabaseManifest(
        ...
        count,
        UInt64(revision),
        index_revision===nothing ? nothing : UInt64(index_revision),
        config,
    )
end
...
function save_manifest(path::AbstractString, manifest::DatabaseManifest)
    ...
    if manifest.format_version>=2
        ...
        manifest.index_revision===nothing||setindex!(
            data,
            Int(manifest.index_revision),
            "index_revision",
        )
    end
...
function load_manifest(path::AbstractString)
    ...
    format_version>=2&&!haskey(data, "revision")&&throw(
        ArgumentError("manifest is missing revision"),
    )
