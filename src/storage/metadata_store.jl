...
function metadata_store_path(
    path::AbstractString,
    filename::AbstractString = "metadata.bin",
)
    isempty(filename)&&throw(ArgumentError("metadata filename cannot be empty"))
    basename(filename)==filename||throw(
        ArgumentError("metadata filename must be a basename"),
    )
    return joinpath(path, filename)
end

function save_metadata_store(
    path::AbstractString,
    store::MetadataStore;
    filename::AbstractString = "metadata.bin",
)
        ...
        try
            ...
            if magic==METADATA_STORE_MAGIC_V2
                ...
                version==PORTABLE_VALUE_FORMAT_VERSION||throw(
                    ArgumentError("unsupported metadata format version"),
                )
            ...
            elseif magic==METADATA_STORE_MAGIC_V1
                ...
                for index = 1:count
                    ...
                    value isa NamedTuple||throw(
                        ArgumentError("stored metadata must be a named tuple"),
                    )
