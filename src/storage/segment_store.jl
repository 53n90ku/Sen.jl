...
function load_segment_state(
    ...
    return open(state_path, "r") do io
        try
            read(io, length(SEGMENT_STATE_MAGIC))==SEGMENT_STATE_MAGIC||throw(
                ArgumentError("invalid segment state file"),
            )
            Int(read_portable_uint16(io))==DATABASE_SEGMENT_FORMAT_VERSION||throw(
                ArgumentError("unsupported segment state format version"),
            )
            count=read_portable_length(io, "segment exclusion count")
            count==expected_count||throw(
                DimensionMismatch("segment exclusions do not match vector count"),
            )
            ...
            for index = 1:count
                ...
                value in (UInt8(0), UInt8(1))||throw(
                    ArgumentError("stored segment exclusion is invalid"),
                )
            ...
            for _ = 1:tombstone_count
                id=read_portable_value(io)
                id in tombstone_ids&&throw(
                    ArgumentError("stored segment tombstone is duplicated"),
                )
...
function load_segment_build_config(entry::AbstractDict, count::Int)
    for key in ("nlists", "iterations", "seed", "restarts", "training_count")
        haskey(entry, key)||throw(
            ArgumentError("segment index descriptor is missing $(key)"),
        )
    end
...
function save_database_segments(path::AbstractString, db::VectorDB)
    ...
    isempty(db.immutable_segments)&&error(
        "segmented database has no immutable primary segment",
    )
...
function load_immutable_segment(
    path::AbstractString,
    entry::AbstractDict,
    dim::Int,
    metric::Symbol;
    mmap_vectors::Union{Bool,Symbol},
    ...
    all(key->haskey(entry, key), required)||throw(
        ArgumentError("immutable segment descriptor is incomplete"),
    )
    vector_file=checked_segment_filename(entry["vector_file"], "segment vector filename")
    metadata_file=checked_segment_filename(
        entry["metadata_file"],
    ...
    vector_store.dim==dim||throw(
        DimensionMismatch("segment vector dimension does not match database"),
    )
    count=length(vector_store)
    count==length(metadata_store)==length(id_store)||throw(
        DimensionMismatch("stored segment stores are misaligned"),
    )
    ...
    if haskey(entry, "index_file")
        ...
        size(ivf.centroids, 1)==dim||throw(
            DimensionMismatch("segment index dimension does not match database"),
        )
        sum(length, ivf.lists)==count||throw(
            DimensionMismatch("segment index count does not match vector count"),
        )
        ivf.metric===metric||throw(
            ArgumentError("segment index metric does not match database"),
        )
        build_config=load_segment_build_config(entry, count)
        length(ivf.lists)==build_config.nlists||throw(
            DimensionMismatch(
                "segment index list count does not match build configuration",
            ),
        )
...
function load_active_segment(
    ...
    all(key->haskey(entry, key), required)||throw(
        ArgumentError("active segment descriptor is incomplete"),
    )
    vector_store=load_vector_store(
        path;
        filename = checked_segment_filename(entry["vector_file"], "active vector filename"),
        mmap = false,
    )
    metadata_store=load_metadata_store(
        path;
        filename = checked_segment_filename(
            entry["metadata_file"],
            "active metadata filename",
        ),
    )
    id_store=load_id_store(
        path;
        filename = checked_segment_filename(entry["id_file"], "active id filename"),
    )
    vector_store.dim==dim||throw(
        DimensionMismatch("active segment vector dimension does not match database"),
    )
    count=length(vector_store)
    count==length(metadata_store)==length(id_store)||throw(
        DimensionMismatch("stored active segment stores are misaligned"),
    )
    validate_stored_vectors!(stored_vectors(vector_store), metric)
...
function validate_loaded_segment_order(
    segments::Vector{ImmutableSegment},
    active::ActiveSegment,
    revision::UInt64,
)
    isempty(segments)&&throw(
        ArgumentError("segment manifest has no immutable primary segment"),
    )
    first(segments).revision_start==UInt64(0)||throw(
        ArgumentError("primary segment must begin at revision zero"),
    )
    previous_end=first(segments).revision_end

    for segment in Iterators.drop(segments, 1)
        segment.revision_start==next_segment_revision(previous_end)||throw(
            ArgumentError("immutable segment revisions are not contiguous"),
        )
        previous_end=segment.revision_end
    end

    if active_segment_is_empty(active)
        active.revision_start==next_segment_revision(previous_end)||throw(
            ArgumentError("empty active segment does not follow immutable segments"),
        )
        active.revision_end==revision||throw(
            ArgumentError("empty active segment revision does not match database"),
        )
        previous_end==revision||throw(
            ArgumentError("immutable segments do not reach database revision"),
        )
    else
        active.revision_start==next_segment_revision(previous_end)||throw(
            ArgumentError("active segment does not follow immutable segments"),
        )
        active.revision_end==revision||throw(
            ArgumentError("active segment does not reach database revision"),
        )
    end
...
function load_database_segments(
    path::AbstractString,
    dim::Int,
    metric::Symbol,
    ...
    for key in
        ("format_version", "database_revision", "immutable_segments", "active_segment")
        haskey(data, key)||throw(ArgumentError("segment manifest is missing $(key)"))
    end

    Int(data["format_version"])==DATABASE_SEGMENT_FORMAT_VERSION||throw(
        ArgumentError("unsupported segment manifest format version"),
    )
    revision=checked_segment_revision(
        data["database_revision"],
        "segment database revision",
    )
    revision==expected_revision||throw(
        ArgumentError("segment manifest revision does not match database manifest"),
    )
    ...
    index_revision=haskey(data, "index_revision") ?
                   checked_segment_revision(
        data["index_revision"],
        "segment index revision",
    ) : nothing
    primary=first(segments)
    (primary.index===nothing)===(index_revision===nothing)||throw(
        ArgumentError("segment index revision does not match primary index"),
    )
    index_revision===nothing||index_revision<=revision||throw(
        ArgumentError("segment index revision is ahead of database"),
    )
    length(Set(segment.id for segment in segments))==length(segments)||throw(
        ArgumentError("immutable segment ids must be unique"),
    )
    active.id in Set(segment.id for segment in segments)&&throw(
        ArgumentError("active segment id duplicates an immutable segment id"),
    )
    return (
