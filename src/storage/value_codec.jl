...
function read_portable_length(io::IO,name::AbstractString;maximum::Int=PORTABLE_MAX_CONTAINER_LENGTH,)
...
function write_portable_text(io::IO,value::AbstractString)
    ...
    length(bytes)<=PORTABLE_MAX_STRING_BYTES||throw(ArgumentError("portable string is too large"))
...
function write_portable_array(io::IO,array::AbstractArray)
    ...
    1<=dimensions<=PORTABLE_MAX_DIMENSIONS||throw(ArgumentError("portable arrays must have between 1 and $(PORTABLE_MAX_DIMENSIONS) dimensions"))
    length(array)<=PORTABLE_MAX_CONTAINER_LENGTH||throw(ArgumentError("portable array is too large"))
...
function read_portable_array_element(io::IO,type::UInt8)
    if type===PORTABLE_TRUE
        value=read(io,UInt8)
        value in (UInt8(0),UInt8(1))||throw(ArgumentError("stored boolean array value is invalid"))
...
function read_portable_array(io::IO)
    ...
    1<=dimensions<=PORTABLE_MAX_DIMENSIONS||throw(ArgumentError("stored portable array dimension count is invalid"))
    ...
    for index in eachindex(shape)
        ...
        count<=PORTABLE_MAX_CONTAINER_LENGTH||throw(ArgumentError("stored portable array is too large"))
    end
...
function is_legacy_julia_serialization_header(bytes::AbstractVector{UInt8})
    return length(bytes)>=length(JULIA_SERIALIZATION_HEADER_PREFIX)&&bytes[1:length(JULIA_SERIALIZATION_HEADER_PREFIX)]==JULIA_SERIALIZATION_HEADER_PREFIX
end
...
function portable_read_error(error,message::AbstractString)
    ...
    error isa InexactError&&throw(ArgumentError("$(message) contains an out-of-range value"))
