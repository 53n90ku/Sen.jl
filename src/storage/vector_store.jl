...
VectorStore(dim::Int, vectors::Matrix{Float32}, count::Int) =
    VectorStore(dim, vectors, count, false)
...
function insert_vector!(store::VectorStore, vector::AbstractVector{<:Real})
    store.mapped&&throw(ArgumentError("memory mapped vector store is read only"))
    length(vector)==store.dim||throw(
        DimensionMismatch("vector dimension doesnt match store"),
    )

    new_index=store.count+1
    ensure_capacity!(store, new_index)

    for dimension = 1:store.dim
...
function read_vector_store_header(io::IO, vector_path::AbstractString)
    ...
    try
        ...
        filesize(vector_path)==expected_bytes||throw(
            ArgumentError("vector file size does not match its header"),
        )
        ...
        error isa OverflowError&&throw(
            ArgumentError("vector file dimensions are too large"),
        )
        rethrow()
    end
end

function should_mmap_vector_store(
    mode::Union{Bool,Symbol},
    payload_bytes::Int,
    threshold_bytes::Int,
)
...
function save_vector_store(
    path::AbstractString,
    store::VectorStore;
    filename::AbstractString = "vectors.bin",
)
    mkpath(path)
        ...
        if should_mmap_vector_store(mmap, header.payload_bytes, mmap_threshold_bytes)
            try
                vectors=Mmap.mmap(
                    io,
                    Matrix{Float32},
                    (header.dim, header.count),
                    VECTOR_STORE_HEADER_BYTES;
                    grow = false,
                    shared = true,
                )
