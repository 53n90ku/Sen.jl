...
function database_wal_is_active(db::VectorDB)
    return isfile(database_wal_path(db.path))||isfile(database_current_path(db.path))||isfile(
        joinpath(db.path, "manifest.toml"),
    )
end
...
function write_database_wal_header(io::IO, revision::UInt64, dim::Int, metric::Symbol)
    ...
    length(body)==DATABASE_WAL_HEADER_BODY_BYTES||error(
        "database WAL header size is invalid",
    )
    write(io, body)
...
function read_database_wal_header(io::IO)
    try
        ...
        length(body)==DATABASE_WAL_HEADER_BODY_BYTES||throw(
            ArgumentError("database WAL header is truncated"),
        )
        checksum=read(io, DATABASE_WAL_CHECKSUM_BYTES)
        length(checksum)==DATABASE_WAL_CHECKSUM_BYTES||throw(
            ArgumentError("database WAL header is truncated"),
        )
        sha256(body)==checksum||throw(
            ArgumentError("database WAL header checksum does not match"),
        )
        ...
        version==DATABASE_WAL_FORMAT_VERSION||throw(
            ArgumentError("unsupported database WAL format version"),
        )
...
function ensure_database_wal(db::VectorDB)
    ...
    if db.wal_revision!==nothing
        db.wal_revision==db.revision||throw(
            ArgumentError("database WAL is ahead of memory; reload the database"),
        )
        return wal_path
    end
    ...
    last_revision==db.revision||throw(
        ArgumentError("database WAL is ahead of memory; reload the database"),
    )
...
function write_database_wal_put_body(io::IO, revision::UInt64, vectors, metadata, ids)
    count=length(ids)
    length(vectors)==count||throw(
        DimensionMismatch("WAL vector count doesnt match id count"),
    )
    length(metadata)==count||throw(
        DimensionMismatch("WAL metadata count doesnt match id count"),
    )
    write_portable_uint64(io, revision)
...
function record_database_mutation!(db::VectorDB, revision::UInt64, body::Vector{UInt8})
    revision==db.revision||throw(
        ArgumentError("mutation history revision doesnt match database"),
    )

    if !isempty(db.mutation_history)
        last(db.mutation_history).revision<revision||throw(
            ArgumentError("mutation history revisions must increase"),
        )
    end
...
function append_database_wal_body!(db::VectorDB, revision::UInt64, body::Vector{UInt8})
    length(body)<=DATABASE_WAL_MAX_RECORD_BYTES||throw(
        ArgumentError("database WAL record is too large"),
    )
    ensure_database_writer_ownership(db)
    wal_path=ensure_database_wal(db)
    revision==next_database_revision(db)||throw(
        ArgumentError("database WAL append revision is invalid"),
    )
...
function append_database_wal_body_if_active!(
    db::VectorDB,
    revision::UInt64,
    body::Vector{UInt8},
)
...
function append_database_wal_put!(db::VectorDB, revision::UInt64, vectors, metadata, ids)
    ...
    return append_database_wal_body_if_active!(db, revision, body) ?
           database_wal_path(db.path) : nothing
end
    ...
    try
        ...
        if operation==DATABASE_WAL_PUT
            ...
            for index = 1:count
                ...
                vector isa Vector{Float32}||throw(
                    ArgumentError("stored WAL vector is invalid"),
                )
                length(vector)==dim||throw(
                    DimensionMismatch("stored WAL vector dimension doesnt match database"),
                )
                ids[index]=id
                push!(vectors, vector)
...
function read_database_wal(path::AbstractString; repair_tail::Bool = false)
    ...
    header=open(wal_path, "r") do io
        ...
        while position(io)<file_size
            ...
            body_length<=UInt64(DATABASE_WAL_MAX_RECORD_BYTES)||throw(
                ArgumentError("stored database WAL record is too large"),
            )
            ...
            checksum=read(io, DATABASE_WAL_CHECKSUM_BYTES)
            sha256(body)==checksum||throw(
                ArgumentError("database WAL checksum does not match"),
            )
            record=read_database_wal_record(body, header.dim)
            expected_revision==typemax(UInt64)&&throw(
                ArgumentError("database WAL revision overflow"),
            )
            expected_revision+=UInt64(1)
            record.revision==expected_revision||throw(
                ArgumentError("database WAL revisions are not contiguous"),
            )
            push!(records, record)
...
function apply_database_wal_record!(db::VectorDB, record::DatabaseWALRecord)
    record.revision==db.revision+UInt64(1)||throw(
        ArgumentError("database WAL revision doesnt follow snapshot"),
    )

    if record.operation==DATABASE_WAL_PUT
        length(Set{Any}(record.ids))==length(record.ids)||throw(
            ArgumentError("database WAL put ids must be unique"),
        )
    ...
    elseif record.operation==DATABASE_WAL_DELETE
        length(Set{Any}(record.ids))==length(record.ids)||throw(
            ArgumentError("database WAL delete ids must be unique"),
        )

        for id in record.ids
            has_database_id(db, id)||throw(
                ArgumentError("database WAL deletes a missing id"),
            )
    ...
    body=record.operation==DATABASE_WAL_PUT ?
         database_wal_put_body(
        record.revision,
        record.vectors,
        record.metadata,
        record.ids,
    ) : database_wal_delete_body(record.revision, record.ids)
...
function database_checkpoint_due(db::VectorDB)
    ...
    operations_due=db.checkpoint_operations>0&&db.revision-checkpoint_revision>=UInt64(
        db.checkpoint_operations,
    )
    bytes_due=false

    if db.checkpoint_bytes>0
        wal_path=database_wal_path(db.path)
        wal_bytes=isfile(wal_path) ? max(0, filesize(wal_path)-DATABASE_WAL_HEADER_BYTES) :
                  0
...
function maybe_checkpoint_database!(db::VectorDB)
    ...
    try
        ...
        @warn "automatic database checkpoint failed; WAL remains durable" exception=(
            error,
            catch_backtrace(),
        )
        return false
