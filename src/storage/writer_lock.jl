...
function acquire_database_writer_lock(path::AbstractString)
    Sys.isunix()||throw(ArgumentError("durable writer ownership is currently supported on Unix systems"))
    mkpath(path)
    lock_path=database_writer_lock_path(path)
    io=open(lock_path,"a+")
    result=ccall(:flock,Cint,(Cint,Cint),fd(io),DATABASE_LOCK_EXCLUSIVE|DATABASE_LOCK_NONBLOCKING)
...
function attach_database_writer_lock!(db::VectorDB,io::IOStream)
    ...
    db.writer_lock===nothing||throw(ArgumentError("database handle already owns a writer lock"))
    db.writer_lock=io
...
function ensure_database_writer_ownership(db::VectorDB)
    ...
    db.writer_lock===nothing&&throw(ArgumentError("database handle does not own the durable writer lock"))
...
function activate_database_writer_lock!(db::VectorDB)
    ...
    try
        ...
        any(isfile,(wal_path,current_path,manifest_path))&&throw(ArgumentError("database already exists; load it instead"))
