...
@testset "streaming score oracle" begin
    ...
    metadata=[(number = index,) for index = 1:300]
...
@testset "blocked exact score oracle" begin
    ...
    metadata=[(number = index,) for index = 1:1_024]
    query=randn(rng, Float32, 64)
    norms=compute_vector_norms(vectors)
    scalar_scores=Float32[
        column_dot(query, vectors, index)/(vector_norm(query)*norms[index]) for
        index = 1:1_024
    ]
...
@testset "search allocation ceilings" begin
    ...
    metadata=[(group = index%5,) for index = 1:count]
    ...
    batch_allocated=@allocated search(
        db,
        queries;
        k = 10,
        nprobe = 8,
...
@testset "blocked matrix exact search" begin
    ...
    metadata=[(group = index%7,) for index = 1:1_200]
    ...
    scalar_results=[
        search(db, @view(queries[:, index]); k = 12, strategy = :exact) for
        index in axes(queries, 2)
    ]
    ...
    @test all(
        index->[result.score for result in matrix_results[index]]≈[result.score for result in scalar_results[index]],
        axes(queries, 2),
    )
    ...
    @test all(
        index->[result.score for result in dirty_matrix[index]]≈[result.score for result in dirty_scalar[index]],
        axes(queries, 2),
    )
