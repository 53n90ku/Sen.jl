...
@testset "expression exact and IVF filtering" begin
    ...
    indexed_exact=search_exact(
        ...
        metric = :dot,
        filter = boolean_filter,
        filter_index = metadata_index,
    )
    legacy_exact=search_exact(
        vectors,
        metadata,
        query;
        k = 5,
    ...
    candidates=filter_ivf_candidates(
        collect(1:5),
        metadata,
        range_filter;
        filter_index = metadata_index,
    ...
    prefiltered=search_ivf_prefilter(
        ivf,
        vectors,
        metadata,
        query;
    ...
    postfiltered=search_ivf_postfilter(
        ivf,
        vectors,
        metadata,
        query;
    ...
    unfiltered=search_exact(
        vectors,
        metadata,
        query;
        k = 5,
        metric = :dot,
        filter_index = build_bitset_index(metadata[1:4]),
    )
    @test [result.index for result in unfiltered]==collect(1:5)
    @test_throws DimensionMismatch search_exact(
        vectors,
        metadata,
        query;
        k = 5,
    ...
    @test_throws DimensionMismatch filter_ivf_candidates(
        collect(1:5),
        metadata,
        range_filter;
        filter_index = build_bitset_index(metadata[1:4]),
