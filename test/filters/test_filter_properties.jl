...
function filter_test_range(value, lower, upper)
    ...
    temporal=value isa Union{Date,DateTime}||lower isa Union{Date,DateTime}||upper isa
                                                                             Union{
        Date,
        DateTime,
    }
...
filter_test_matches(row::NamedTuple, expr::FilterTestAnd) =
    all(child->filter_test_matches(row, child), expr.children)
filter_test_matches(row::NamedTuple, expr::FilterTestOr) =
    any(child->filter_test_matches(row, child), expr.children)
filter_test_matches(row::NamedTuple, expr::FilterTestNot) =
    !filter_test_matches(row, expr.child)
...
function random_filter_atom(rng::AbstractRNG)
    ...
    elseif choice==4
        ...
        return (
            Sen.Range(:floating, lower, upper),
            FilterTestRange(:floating, lower, upper),
        )
    elseif choice==5
    ...
    elseif choice==8
        ...
        return (
            Sen.Range(:datetime, lower, upper),
            FilterTestRange(:datetime, lower, upper),
        )
    end
...
function random_filter_expr(rng::AbstractRNG, depth::Int = 0)
    ...
    if choice<=7
        count=rand(rng, 0:3)
        pairs=[random_filter_expr(rng, depth+1) for _ = 1:count]
        return (
            Sen.And((pair[1] for pair in pairs)...),
            FilterTestAnd(Tuple(pair[2] for pair in pairs)),
        )
    elseif choice<=9
        count=rand(rng, 0:3)
        pairs=[random_filter_expr(rng, depth+1) for _ = 1:count]
        return (
            Sen.Or((pair[1] for pair in pairs)...),
            FilterTestOr(Tuple(pair[2] for pair in pairs)),
        )
    end
...
@testset "filter expression boolean semantics" begin
    ...
    expr=Sen.And(
        Sen.Eq(:group, "a"),
        Sen.Or(Sen.Range(:score, 2, 4), Sen.Eq(:active, false)),
...
@testset "filter expression identity stability" begin
    left=Sen.And(
        Sen.Eq(:group, "a"),
        Sen.Or(Sen.Eq(:score, 1), Sen.Not(Sen.Eq(:active, false))),
    )
    right=Sen.And(
        Sen.Eq(:group, "a"),
        Sen.Or(Sen.Eq(:score, 1), Sen.Not(Sen.Eq(:active, false))),
    ...
    @test Sen.normalize_filter((group = "a", score = 1))==Sen.And(
        Sen.Eq(:group, "a"),
        Sen.Eq(:score, 1),
    )
...
@testset "filter expression complexity limits" begin
    ...
    @test_throws ArgumentError begin
        children=Tuple(Sen.Eq(:value, index) for index = 1:10_000)
