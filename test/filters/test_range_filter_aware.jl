...
@testset "filter-aware list ranges" begin
    ...
    for filter in filters
        expected_mask=BitVector(
            matches_filter(metadata[global_id], filter) for global_id in ivf.lists[1]
        )
        expected_ids=Int[
            ivf.lists[1][position] for
            position in eachindex(expected_mask) if expected_mask[position]
        ]
        @test evaluate_list_filter(index, 1, filter)==expected_mask
        @test estimate_list_filter_count(index, 1, filter)==count(expected_mask)
        @test estimate_list_filter_density(index, 1, filter)==count(expected_mask)/length(
            expected_mask,
        )
