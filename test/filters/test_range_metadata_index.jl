...
@testset "ordered numeric range lanes match scalar semantics" begin
    ...
    @test sum(
        length(lane.values) for lanes in values(index.range_lanes) for lane in lanes
    )==10
...
@testset "Date and DateTime range lanes stay strict" begin
    ...
    @test evaluate_filter(index, date_filter)==range_oracle(metadata, date_filter)==BitVector([
        true,
        false,
        true,
        false,
    ...
    @test evaluate_filter(index, datetime_filter)==range_oracle(metadata, datetime_filter)==BitVector([
        false,
        true,
        false,
        false,
...
@testset "high cardinality ranges remain linear" begin
    count=10_000
    index=build_bitset_index(NamedTuple[(number = position,) for position = 1:count])
    ...
    @test evaluate_filter(index, Range(:number, 4_500, 5_500))==BitVector([
        4_500<=position<=5_500 for position = 1:count
    ])
end
