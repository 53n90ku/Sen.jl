...
@testset "automatic sealing and background segment indexing" begin
    ...
    try
        ...
        @test [
            result.id for
            result in search(db, Float32[1, 0]; k = 4, nprobe = 1, strategy = :ivf)
        ]==[1, 2, 3, 4]
        ...
        @test [
            result.id for
            result in search(db, Float32[1, 0]; k = 5, nprobe = 1, strategy = :ivf)
        ]==[1, 2, 3, 4, 5]
    finally
...
@testset "segment indexing failure keeps exact fallback" begin
    config=MaintenanceConfig(
        ...
        active_segment_threshold = 1,
        incremental_indexing = true,
        max_retries = 0,
        persist_after_rebuild = false,
    )
    db=create_db(
        "segment-index-failure";
        dim = 2,
        metric = :dot,
        durable = false,
        maintenance_config = config,
    ...
    @test [
        result.id for
        result in search(db, Float32[1, 0]; k = 2, nprobe = 1, strategy = :ivf)
    ]==[1, 2]
...
@testset "segment indexing resumes and persists" begin
    mktempdir() do path
        ...
        @test [
            result.id for
            result in search(reopened, Float32[1, 0]; k = 4, nprobe = 1, strategy = :ivf)
        ]==[1, 2, 3, 4]
        close(reopened)
    end
end
...
@testset "immutable segment lifecycle" begin
    ...
    insert!(
        db,
        Float32[1 0.8 0.6 0.4; 0 0 0 0; 0 0 0 0],
        [
            (group = "red", version = 1),
            (group = "blue", version = 1),
            (group = "red", version = 1),
            (group = "blue", version = 1),
        ];
        ids = ["a", "b", "c", "d"],
    ...
    @test [
        result.id for result in search(db, Float32[1, 0, 0]; k = 10, strategy = :exact)
    ]==["a", "e", "c", "d"]
    @test [
    ...
    expected_red=materialized_exact_results(
        db,
        query;
        k = 10,
        filter = Sen.Eq(:group, "red"),
    ...
    @test [[result.id for result in results] for results in batch]==[
        [
            result.id for
            result in search(db, @view(queries[:, index]); k = 10, strategy = :exact)
...
@testset "segment persistence and WAL replay" begin
    mktempdir() do path
        ...
        @test [
            result.id for
            result in search(loaded, Float32[1, 0, 0]; k = 10, strategy = :exact)
        ]==expected_ids
        @test get_record(loaded, "a").metadata.version==2
        @test_throws KeyError get_record(loaded, "b")
        @test all(
            segment->length(segment.vector_store)==0||Sen.is_mapped(segment.vector_store),
            loaded.immutable_segments,
        )
        ...
        @test [
            result.id for
            result in search(reopened, Float32[1, 0, 0]; k = 10, strategy = :exact)
        ]==expected_ids
        close(reopened)
    end
end
    ...
    for (metric, seed) in ((:cosine, 503), (:dot, 504))
        ...
        for step = 1:60
            ...
            if step%10==0
                ...
                group=rand(rng, 1:3)
                expected_filtered=materialized_exact_results(
                    db,
                    query;
                    k = k,
                    filter = Sen.Eq(:group, group),
                )
                actual_filtered=search(
                    db,
                    query;
                    k = k,
                    filter = Sen.Eq(:group, group),
