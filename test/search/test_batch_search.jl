...
@testset "batch search workers" begin
    ...
    metadata=[(group = index%4, name = "record-$(index)") for index = 1:160]
    ids=collect(1:160)
    ...
    @test Sen.resolve_database_batch_workers(
        24;
        parallel = false,
        workers = 3,
        parallel_threshold = 2,
    ...
    @test Sen.resolve_database_batch_workers(
        ...
        workers = 3,
        parallel_threshold = 2,
    )==min(3, Threads.nthreads(:default))
    ...
    @test isempty(
        search(db, empty_queries; parallel = true, workers = 3, parallel_threshold = 2),
    )
    ...
    @test_throws ArgumentError search(
        db,
        queries;
        k = 0,
        parallel = true,
    ...
    @test_throws DimensionMismatch search(
        db,
        zeros(Float32, 7, 4);
        parallel = true,
        workers = 3,
...
@testset "parallel batch and writer" begin
    ...
    metadata=[(group = index%5,) for index = 1:400]
