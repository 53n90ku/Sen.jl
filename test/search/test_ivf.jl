...
@testset "ivf postfilter oversampling" begin
    ...
    metadata=[(allowed = false,), (allowed = false,), (allowed = true,), (allowed = true,)]

    centroids=reshape(Float32[1.0, 0.0], 2, 1)
    index=IVFIndex(centroids, [[1, 2, 3, 4]])
    query=Float32[1.0, 0.0]

    without_oversampling=search_ivf_postfilter(
        index,
        vectors,
        metadata,
        query;
    ...
    with_oversampling=search_ivf_postfilter(
        index,
        vectors,
        metadata,
        query;
    ...
    @test_throws ArgumentError search_ivf_postfilter(
        index,
        vectors,
        metadata,
        query;
...
@testset "ivf search" begin
    ...
    metadata =
        [(name = "left-1",), (name = "left-2",), (name = "right-1",), (name = "right-2",)]
    index = build_ivf(vectors; nlists = 2, iterations = 20, seed = 42)
    query = Float32[-1, 0]
...
@testset "ivf post filter search" begin
    ...
    empty_results = search_ivf_postfilter(
        index,
        vectors,
        metadata,
        query;
...
@testset "filter aware ivf stats" begin
    ...
    index =
        build_filter_aware_ivf(vectors, metadata; nlists = 2, iterations = 20, seed = 42)
    ...
    query = Float32[-1, 0]
    vector_only_ranking = rank_filter_aware_lists(
        index,
        query,
        (side = "right",);
        nprobe = 1,
    ...
    filter_only_ranking = rank_filter_aware_lists(
        index,
        query,
        (side = "right",);
        nprobe = 1,
    ...
    @test_throws ArgumentError rank_filter_aware_lists(
        index,
        query,
        (side = "right",);
        nprobe = 3,
...
@testset "compound filter density" begin
    ...
    metadata=[(first = true, second = false), (first = false, second = true)]
...
@testset "filter aware ivf search" begin
    ...
    metadata = [(side = "left",), (side = "left",), (side = "right",), (side = "right",)]

    index =
        build_filter_aware_ivf(vectors, metadata; nlists = 2, iterations = 20, seed = 42)
    query = Float32[-1, 0]
    naive_results = search_ivf_postfilter(
        index.ivf,
        vectors,
        metadata,
        query;
    ...
    aware_results = search_filter_aware_ivf(
        index,
        vectors,
        metadata,
        query;
    ...
    @test all(result.metadata.side == "right" for result in aware_results)
end
...
@testset "adaptive filter aware ivf" begin
    ...
    adaptive=search_filter_aware_ivf(
        index,
        vectors,
        metadata,
        query;
    ...
    selection=Sen.select_filter_aware_candidates(
        index,
        metadata,
        query,
        (side = "right",);
...
@testset "spherical ivf geometry" begin
    ...
    @test all(
        list_index->isapprox(
            vector_norm(@view index.centroids[:, list_index]),
            1.0f0;
            atol = 1.0f-5,
        ),
        1:6,
    ...
    @test_throws ArgumentError train_centroids(
        dataset.vectors;
        nlists = 6,
        training_count = 5,
    )
...
@testset "safe bound search" begin
    ...
    @test test_recall_at_k(
        [result.index for result in stats.results],
        [result.index for result in truth],
        10,
    )==1.0
    @test stats.exact
