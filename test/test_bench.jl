...
@testset "synthetic dataset" begin
    ...
    @test dataset.metadata[1].topic in ("systems", "machine-learning", "databases")
end
...
@testset "batch search benchmark" begin
    ...
    @test_throws ArgumentError run_batch_search_benchmark(
        db,
        zeros(Float32, 4, 0);
        repetitions = 1,
    )
...
@testset "clustered dataset" begin
    ...
    query_set=generate_clustered_queries(
        10,
        dataset.centers;
        query_noise = 0.05,
        cluster_skew = 1.0,
    ...
    @test all(
        index->isapprox(sum(abs2, dataset.vectors[:, index]), 1.0f0; atol = 1.0f-5),
        1:100,
    )
    @test size(query_set.queries)==(8, 10)
    @test all(cluster->1<=cluster<=4, query_set.assignments)
    @test dataset.vectors==generate_clustered_dataset(
        100,
        8;
        clusters = 4,
        cluster_noise = 0.05,
...
@testset "exact ground truth" begin
    ...
    filtered_truth = compute_groundtruth(
        vectors,
        metadata,
        queries[:, 1:1];
        k = 1,
...
@testset "candidate counts" begin
    ...
    prefilter_work=count_ivf_prefilter_work(
        index.ivf,
        metadata,
        query,
        (side = "right",);
    ...
    @test count_filter_aware_candidates(
        index,
        metadata,
        query,
        (side = "right",);
...
@testset "selectivity metadata" begin
    ...
    @test all(!sparse[index].selected||dense[index].selected for index = 1:100)
...
@testset "filter workloads" begin
    ...
    for workload in (:random, :correlated, :anticorrelated, :skewed)
        sparse=generate_filter_metadata(
            dataset.vectors,
            0.10;
            workload = workload,
            seed = 43,
        )
        dense=generate_filter_metadata(
            dataset.vectors,
            0.50;
            workload = workload,
            seed = 43,
...
@testset "selectivity sweep" begin
    ...
    @test_throws ArgumentError run_selectivity_sweep(
        dataset.vectors,
        queries,
        Float64[];
        nlists = 4,
...
@testset "nprobe sweep" begin
    ...
    @test all(
        point->point.candidates_visited>=point.candidates_scored,
        sweep_points(results, :ivf_prefilter),
    )
    ...
    @test_throws ArgumentError run_nprobe_sweep(
        dataset.vectors,
        metadata,
        queries,
        filters,
