...
@testset "database persistence" begin
    mktempdir() do path
        ...
        results=search(loaded,[-1.0,0.0];k=2,nprobe=1,filter=(side="right",),strategy=:filter_aware,vector_weight=0.0,filter_weight=1.0,)
    ...
    mktempdir() do path
        ...
        @test [result.id for result in search(loaded,[1.0,0.0];k=2,strategy=:exact,)]==["right","left"]

        save!(loaded;retain_snapshots=1,)
        @test is_mapped(mapped_store)
        @test [result.id for result in search(loaded,[-1.0,0.0];k=1,strategy=:ivf,nprobe=1,)]==["left"]
