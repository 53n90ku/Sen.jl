...
@testset "exact search" begin
    ...
    filtered_results = search_exact(vectors,metadata,query;k=2,filter=(name="up",),)
    @test length(filtered_results)==1
    @test filtered_results[1].index==2
    @test filtered_results[1].metadata.name=="up"

    empty_results = search_exact(vectors, metadata,query;k=2,filter=(name="missing",),)
    @test isempty(empty_results)

    bitset_results=search_exact(vectors,metadata,query;k=2,filter=(name="up",),filter_index=filter_index,)
