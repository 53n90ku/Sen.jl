...
@testset "expression workload generation" begin
    ...
    @test length(workload.cases)==length(EXPRESSION_FAMILIES)*length(keys(EXPRESSION_BUCKETS))
...
@testset "expression benchmark compatibility" begin
    ...
    workload=generate_expression_workload(vectors;workload=:correlated,seed=214,families=(:range_numeric,:and,:not),buckets=(:medium,))
    queries=vectors[:,1:2]

    for case in workload.cases
        filters=fill(case.filter,size(queries,2))
        truth=compute_groundtruth(vectors,workload.metadata,queries;k=5,filters=filters,)
        result=run_benchmark(vectors,workload.metadata,queries,filters;nlists=4,k=5,nprobe=4,iterations=3,seed=215,repetitions=1,)
