...
@testset "expression selectivity" begin
    ...
    expressions=[
        ...
        Sen.And(Sen.Or(Sen.Eq(:group,"a"),Sen.Eq(:group,"c")),Sen.Not(Sen.Eq(:active,false))),
        Sen.And(),
        Sen.Or(),
    ]
    ...
    @test estimate_selectivity(index,legacy)==estimate_selectivity(index,Sen.normalize_filter(legacy))
    ...
    @test_throws DimensionMismatch estimate_selectivity(index,range;metadata=metadata[1:2],)
...
@testset "bounded expression search" begin
    ...
    index=build_filter_aware_ivf(vectors,metadata;nlists=2,iterations=5,seed=73,metric=:cosine,)
    expressions=[
        Sen.Eq(:group,"a"),
        Sen.In(:group,["a","c"]),
        Sen.And(Sen.In(:group,["a","b"]),Sen.Eq(:active,true)),
    ...
    for expression in expressions
        expected=search_exact(vectors,metadata,query;k=8,metric=:cosine,filter=expression,)
    ...
    legacy=(group="a",active=true,)
    legacy_results=search_filter_aware_bound(index,vectors,metadata,query;filter=legacy,k=8,max_nprobe=length(index.ivf.lists),)
    expression_results=search_filter_aware_bound(index,vectors,metadata,query;filter=Sen.normalize_filter(legacy),k=8,max_nprobe=length(index.ivf.lists),)
    @test [result.index for result in legacy_results]==[result.index for result in expression_results]

    range=Sen.Range(:score,2,4)
    range_expected=search_exact(vectors,metadata,query;k=8,metric=:cosine,filter=range,)
    range_results=search_filter_aware_bound(index,vectors,metadata,query;filter=range,k=8,max_nprobe=length(index.ivf.lists),)
