...
@testset "filter aware metadata indexes evaluate expressions" begin
    ...
    @test all(
        list_index->index.metadata_indexes[list_index].count==length(ivf.lists[list_index]),
        eachindex(ivf.lists),
    )
    ...
    for filter in filters
        for list_index in eachindex(ivf.lists)
            ...
            expected_mask=BitVector(
                matches_filter(metadata[vector_index], filter) for vector_index in list
            )
            expected_candidates=Int[
                list[position] for position in eachindex(list) if expected_mask[position]
            ]

            @test evaluate_list_filter(index, list_index, filter)==expected_mask
            @test estimate_list_filter_count(index, list_index, filter)==count(
                expected_mask,
            )
            @test estimate_list_filter_density(index, list_index, filter)==count(
                expected_mask,
            )/length(list)
        ...
        expected=vcat(
            (
                filtered_list_candidates(index, list_index, filter) for list_index in (2, 1)
            )...,
        )
    ...
    results=search_filter_aware_ivf(
        index,
        vectors,
        metadata,
        Float32[1.0, 0.0];
    ...
    @test Set(result.index for result in results)==Set(
        index for index in eachindex(metadata) if matches_filter(metadata[index], filter)
    )
end
...
@testset "list metadata indexes evaluate ranges" begin
    ...
    @test filtered_list_candidates(
        index,
        1,
        And(Eq(:year, 1900), Range(:year, 2025, 2026)),
    )==Int[]
end
