...
using .FilterExprContract: normalize_filter,validate_filter,matches_filter,filter_value_isequal
...
@testset "filter expression validation" begin
    ...
    node_limit=And(Tuple(Eq(:value,index) for index in 1:(MAX_FILTER_NODES-1)))
...
@testset "filter scalar evaluation" begin
    metadata=(language="julia",year=2026,score=NaN,optional=missing,zero=0.0,date=Date(2026,7,15),created_at=DateTime(2026,7,15,10,30))
    ...
    @test matches_filter(metadata,Range(:created_at,DateTime(2026,7,15),DateTime(2026,7,16)))
    @test !matches_filter(metadata,Range(:date,DateTime(2026,1,1),DateTime(2026,12,31)))
