...
@testset "adaptive metadata index expressions" begin
    metadata=NamedTuple[(group=index<=64 ? "common" : "other",tag=index,) for index in 1:128]
    ...
    @test evaluate_filter(index,Eq(:group,"common"))==BitVector([position<=64 for position in 1:128])
    @test evaluate_filter(index,In(:tag,[1,3,128]))==BitVector([position in (1,3,128) for position in 1:128])
    @test evaluate_filter(index,And(Eq(:group,"common"),Not(In(:tag,[1,2]))))==BitVector([3<=position<=64 for position in 1:128])
    @test evaluate_filter(index,Or(Eq(:tag,1),Eq(:tag,128)))==BitVector([position in (1,128) for position in 1:128])
    @test evaluate_filter(index,(group="other",tag=128,))==BitVector([position==128 for position in 1:128])

    owned=evaluate_filter(index,Eq(:group,"common"))
    owned[1]=false
    @test evaluate_filter(index,Eq(:group,"common"))[1]

    @test evaluate_filter(index,Range(:tag,1,10))==BitVector([1<=position<=10 for position in 1:128])
end

@testset "temporal equality keeps Date and DateTime distinct" begin
    date=Date(2026,7,15)
    datetime=DateTime(2026,7,15)
    metadata=NamedTuple[(created=date,),(created=datetime,),(created=Date(2026,7,16),)]
    index=build_bitset_index(metadata)

    for filter in (Eq(:created,date),Eq(:created,datetime),In(:created,[date,datetime]))
...
@testset "high cardinality metadata stays sparse" begin
    count=2_000
    index=build_bitset_index(NamedTuple[(unique_id=position,) for position in 1:count])
