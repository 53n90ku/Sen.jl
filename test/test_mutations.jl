...
@testset "database mutations" begin
    ...
    state=(revision=db.revision,vectors=copy(stored_vectors(db.vector_store)),metadata=copy(stored_metadata(db.metadata_store)),ids=copy(stored_ids(db.id_store)),)
...
@testset "mutation state model" begin
    ...
    for step in 1:160
        ...
        @test length(db.delta_store.vector_store)==length(db.delta_store.metadata_store)==length(db.delta_store.id_store)
        live_base=Set(id for id in stored_ids(db.id_store) if !db.base_tombstones[get_position(db.id_store,id)])
        ...
        for(id,expected) in model
            ...
            if has_id(db.delta_store.id_store,id)
                @test get_id(db.delta_store.id_store,get_position(db.delta_store.id_store,id))==id
            else
        ...
        if step%40==0&&!isempty(model)
            ...
            build!(db;nlists=nlists,iterations=3,seed=step,restarts=1,training_count=length(db),)
            query=Float32.(randn(rng,3))
            k=min(5,length(model))
            expected=sort(collect(model);by=pair->cosine_similarity(query,pair.second.vector),rev=true,)[1:k]
