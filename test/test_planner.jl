...
@testset "planner cost model" begin
    ...
    @test estimate_required_nprobe(1000,10,0.10,10;candidate_multiplier=1.0,target_recall=1.0,)==1
    @test estimate_required_nprobe(1000,10,0.01,10;candidate_multiplier=1.0,target_recall=1.0,)==10
...
@testset "strategy selection" begin
    ...
    for index in 1:100
        insert!(db,Float32[index,1.0],(group=index<=10 ? "rare" : "common",unique=index==1,))
    end
    ...
    calibration=RecallCalibration([
        RecallCalibrationEntry(:filter_aware,:random,0.10,100,4,0.90,4,0.95,1.0,true),
        RecallCalibrationEntry(:filter_aware_bound,:random,0.10,100,4,0.90,2,0.95,0.8,true),
    ])
    calibrated_config=PlannerConfig(calibration=calibration,workload=:random,)
    calibrated_estimates=estimate_strategy_costs(db,(group="rare",);k=5,config=calibrated_config,)
    calibrated_plan=plan_query(db,(group="rare",);k=5,strategy=:filter_aware,config=calibrated_config,)
