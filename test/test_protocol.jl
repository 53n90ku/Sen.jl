...
@testset "benchmark protocol" begin
    ...
    @test experiment_methods(spec)==[:exact,:ivf_prefilter,:ivf_postfilter,:filter_aware,:filter_aware_bound]
    @test_throws ArgumentError ExperimentSpec("invalid";vector_count=10,dimension=2,nlists=2,nprobes=Int[],)
    @test_throws ArgumentError ExperimentSpec("invalid";vector_count=10,dimension=2,nlists=2,nprobes=[3],)
    scale_spec=ExperimentSpec("scale-grid";vector_count=1_000_000,dimension=1,nlists=1,nprobes=[1],selectivity=0.05,)
    ...
    ivf=build_ivf(base.vectors;nlists=spec.nlists,iterations=spec.iterations,seed=spec.seed,restarts=spec.restarts,training_count=spec.training_count,)
    postfilter_rankings=build_postfilter_rankings(spec,ivf,base.vectors,base.train_queries)
    @test postfilter_rankings.nprobes==spec.nprobes
    @test length(postfilter_rankings.rankings)==spec.train_query_count
    ranking_metadata=[(position=index,) for index in 1:spec.vector_count]

    for nprobe in spec.nprobes
        query=@view base.train_queries[:,1]
        candidate_count=count_ivf_candidates(ivf,query;nprobe=nprobe,)
        expected=search_ivf(ivf,base.vectors,ranking_metadata,query;k=candidate_count,nprobe=nprobe,)
        @test postfilter_rankings.rankings[1][nprobe]==[result.index for result in expected]
    end

    result=run_claim_benchmark(spec;base=base,prebuilt_ivf=ivf,prebuilt_build_seconds=0.01,prebuilt_postfilter_rankings=postfilter_rankings,)
    ...
    @test all(row->row.postfilter_candidate_multiplier==result.postfilter_candidate_multiplier,result.method_results[:ivf_postfilter].rows)
    ...
    @test only(row.postfilter_candidate_multiplier for row in summary_rows if row.method=="ivf_postfilter")==result.postfilter_candidate_multiplier
...
@testset "frozen quality contract" begin
    ...
    @test all(metric->any(spec->spec.metric===metric&&spec.filter_workload===:none,contract.workloads),(:cosine,:dot,))
