...
function range_property_metadata(rng::AbstractRNG,count::Int)
    numeric_values=Any[-Inf,-4,-1.5,-0.0,0.0,Float32(0.0),1,2.5,UInt64(4),Inf,NaN,missing]
    date_values=Any[Date(2023,1,1),Date(2024,6,1),Date(2025,12,31),missing]
    datetime_values=Any[DateTime(2023,1,1),DateTime(2024,6,1,12),DateTime(2025,12,31,23),missing]
    ...
    for index in 1:count
        ...
        if index%11==0
            push!(metadata,(group=group,active=active,day=day,stamp=stamp,temporal=temporal,))
        elseif index%13==0
            push!(metadata,(group=group,active=active,score=score,temporal=temporal,))
        else
            push!(metadata,(group=group,active=active,score=score,day=day,stamp=stamp,temporal=temporal,))
        end
    end
...
function random_range_property_atom(rng::AbstractRNG)
    ...
    choice==6&&return Sen.Range(:stamp,DateTime(2023,1,1),DateTime(2025,12,31,23,59))
    choice==7&&return Sen.Range(:temporal,Date(2023,1,1),Date(2025,12,31))
    choice==8&&return Sen.Range(:temporal,DateTime(2023,1,1),DateTime(2025,12,31,23,59))
...
function random_range_property_expr(rng::AbstractRNG,depth::Int=0)
    ...
    if choice<=7
        return Sen.And(random_range_property_expr(rng,depth+1),random_range_property_expr(rng,depth+1))
    elseif choice<=9
        return Sen.Or(random_range_property_expr(rng,depth+1),random_range_property_expr(rng,depth+1))
    end
...
@testset "range index edge cases" begin
    ...
    expressions=[
        ...
        Sen.Or(Sen.Range(:day,Date(2025,1,1),Date(2025,1,2)),Sen.Eq(:value,missing)),
    ]
    ...
    datetime_mask=evaluate_filter(index,Sen.Range(:stamp,DateTime(2025,1,1),DateTime(2025,1,3)))
...
@testset "randomized range scalar index oracle" begin
    ...
    index=build_bitset_index(metadata)
    expressions=[random_range_property_expr(rng) for _ in 1:60]
...
@testset "range list index mapping" begin
    ...
    index=build_filter_aware_ivf(vectors,metadata;nlists=4,iterations=4,seed=118,metric=:cosine,)
    expressions=[random_range_property_expr(rng) for _ in 1:60]

    for expression in expressions
        for list_index in eachindex(index.ivf.lists)
            vector_indices=index.ivf.lists[list_index]
            expected=BitVector(Sen.matches_filter(metadata[vector_index],expression) for vector_index in vector_indices)
            mask=evaluate_list_filter(index,list_index,expression)
            candidates=filtered_list_candidates(index,list_index,expression)

            @test mask==expected
            @test candidates==Int[vector_indices[position] for position in eachindex(expected) if expected[position]]
