...
@testset "database WAL replay" begin
    mktempdir() do path
        ...
        upsert!(db,Float32[0.95 0.7;0.0 0.0],[(name="update",version=2,),(name="upsert",version=1,)];ids=["update","upsert"],)
...
@testset "database WAL crash boundaries" begin
    ...
    mktempdir() do path
        ...
        append_database_wal_put!(db,revision,[Float32[0.5,0.0]],[(name="synced",)],["synced"])

        @test_throws ArgumentError insert!(db,[0.25,0.0],(name="stale-memory",);id="stale-memory",)
...
@testset "database WAL rejected mutation" begin
    mktempdir() do path
        ...
        @test_throws ArgumentError insert!(db,[0.0,1.0],(unsupported=Dict("value"=>1),);id="invalid",)
...
@testset "database mutation fault atomicity" begin
    mktempdir() do path
        ...
        @test is_built(db)
        @test_throws ArgumentError insert!(db,[NaN,1.0],(name="invalid",);id="invalid",)
        @test db.revision==revision
        @test filesize(database_wal_path(path))==wal_size

        Sen.inject_database_mutation_fault!(:before_wal)
        @test_throws Sen.InjectedDatabaseMutationError insert!(db,[0.0,1.0],(name="before",);id="before",)
        @test db.revision==revision
        @test length(db)==1
        @test filesize(database_wal_path(path))==wal_size
        ...
        during_apply_error=try
            insert!(db,Float32[-1.0 0.5;0.0 0.5],[(name="batch-a",),(name="batch-b",)];ids=["batch-a","batch-b"],)
            nothing
        catch error
            error
        end
        ...
        @test Set([get_record(db,"batch-a").id,get_record(db,"batch-b").id])==Set(["batch-a","batch-b"])
        @test length(db)==4
        ...
        @test Set(result.id for result in search(reopened,[1.0,0.0];k=length(reopened),strategy=:exact,))==expected_ids
        close(reopened)
    end
    ...
    @test_throws Sen.InjectedDatabaseMutationError insert!(db,Float32[0.0 -1.0;1.0 0.0],[(name="a",),(name="b",)];ids=["a","b"],)
...
@testset "automatic WAL checkpoints" begin
    ...
    mktempdir() do path
        db=create_db(path;dim=2,checkpoint_operations=1,checkpoint_bytes=0,checkpoint_retain_snapshots=1,)
        insert!(db,[1.0,0.0],(name="first",);id="first",)
        insert!(db,[0.8,0.0],(name="second",);id="second",)
        insert!(db,[0.6,0.0],(name="third",);id="third",)
